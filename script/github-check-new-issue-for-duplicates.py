...
STOPWORDS = {
    ...
    "the", "this", "when", "while", "with", "won't", "work", "working", "vela",
}
...
"Attach Vela log file" or "Relevant Vela settings" — these are evidence about the symptom and
...
def parse_duplicate_magnets():
    ...
    # format: ## area_name
    #         -   [N dupes] https://github.com/vela-industries/vela/issues/NUMBER
...
- Two issues about "Copilot models not showing" — one caused by a Vela update breaking the model list,
  the other caused by the user's plan not including those models.
- Two issues about "Vela hangs" — one triggered by network drives, the other by large projects.
...
6. Vague catch-all candidate. A closed issue like "Vela is slow" / "performance" / "agent
