...
These are some notes of how to extract an extension from the main vela repository and generate a new repository which preserves the history as best as possible. In the this example we will be extracting the `ruby` extension, substitute as appropriate.
...
1. Create a clean clone the vela repository, delete tags and delete branches.

```sh
LANGNAME=your_language_name_here

rm -rf $LANGNAME
git clone --single-branch --no-tags git@github.com:vela-industries/vela.git $LANGNAME
cd $LANGNAME
```
...
chore: ==>
vela_extension_api: ==>
"'regex:(?<![\[a-zA-Z0-9])(#[0-9]{3,5})==>vela-industries/vela\1' \
...
Create a new empty repo on github under the [vela-extensions](https://github.com/organizations/vela-extensions/repositories/new) organization.

```
git remote add origin git@github.com:vela-extensions/$LANGNAME
...
perl -i -pe "s#https://github.com/vela-industries/vela#https://github.com/vela-extensions/${LANGNAME}#g" extension.toml
...
7. In vela repository, remove the old extension and push a PR.
...
git commit -m "Migrate to $LANGNAME extension to vela-extensions/$LANGNAME"
git push
gh pr create --web
```
...
git submodule add https://github.com/vela-extensions/${LANGNAME}.git extensions/${LANGNAME}
pnpm sort-extensions

# edit extensions.toml:
# - bump version
# - change `submodule` from `extensions/vela` to new path
...
Create PR and reference the Vela PR with removal from tree.
