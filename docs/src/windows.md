...
You can also build vela from source, see [these docs](https://vela.dev/docs/development/windows) for instructions.
...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for Vela, and click Uninstall.
...
Vela supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from Vela.
...
### Vela fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with Vela. See our [installation guide](./installation.md) for more information.
...
- Try launching Vela with no extensions or custom settings to isolate conflicts.
...
#### Vela fails to open / degraded performance

Vela requires a DirectX 11 compatible GPU to run. If Vela fails to open, your GPU may not meet the minimum requirements.
...
If you're running Vela inside a virtual machine, it will use the emulated adapter provided by your VM. While Vela will work in this environment, performance may be degraded.
