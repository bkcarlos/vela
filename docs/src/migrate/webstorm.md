---
title: How to Migrate from WebStorm to Vela
description: "Guide for migrating from WebStorm to Vela, including settings and keybindings."
---

# How to Migrate from WebStorm to Vela

This guide covers how to set up Vela if you're coming from WebStorm, including keybindings, settings, and the differences you should expect as a JavaScript/TypeScript developer.
...
1. Open Settings with {#kb vela::OpenSettings}
...
Vela also supports per-project settings. Create a `.vela/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in WebStorm.
...
After setup, use {#kb:jetbrains file_finder::Toggle} to open a folder. This becomes your workspace in Vela. Unlike WebStorm, there's no project configuration wizard, no framework selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in Vela. The editor will treat that folder as the root of your project. For new projects, you'd typically run `npm init`, `pnpm create`, or your framework's CLI tool first, then open the resulting folder in Vela.
...
Vela doesn't index. You open a folder and start coding immediately—no progress bars, no "Indexing paused" banners. File search and navigation stay fast regardless of project size or how many `node_modules` dependencies you have.

WebStorm's index enables features like finding all usages across your entire codebase, tracking import hierarchies, and flagging unused exports project-wide. Vela relies on language servers for this analysis, which may not cover as much ground.
...
Vela uses the Language Server Protocol (LSP) for code intelligence. For JavaScript and TypeScript, Vela supports:
...
Vela takes a different approach: a project is just a folder. There's no setup wizard, no framework detection dialog, no project structure to configure.
...
- No framework detection. Vela treats React, Angular, Vue, and vanilla JS/TS the same way.
...
Vela has none of this built-in. The TypeScript language server sees your code as TypeScript—it doesn't understand that a function is a React component or that a file is an Angular service.
...
- Consider using framework-specific CLI tools (`ng`, `next`, `vite`) from Vela's terminal
- For React, JSX/TSX syntax and TypeScript types still provide good intelligence

> **Tip:** For projects with complex configurations, keep your framework's documentation handy. Vela's speed comes with less hand-holding for framework-specific features.

### Tool Windows vs. Docks

WebStorm organizes auxiliary views into numbered tool windows. Vela uses a similar concept called "docks":

| WebStorm Tool Window | Vela Equivalent | Vela Keybinding                             |
| -------------------- | -------------- | ------------------------------------------ |
...
Note that there's no dedicated npm tool window in Vela. Use the terminal or define tasks for your common npm scripts.

### Debugging

Both WebStorm and Vela offer integrated debugging for JavaScript and TypeScript:

- Vela uses `vscode-js-debug` (the same debug adapter that VS Code uses)
...
Vela also recognizes `.vscode/launch.json` configurations, so existing VS Code debug setups often work out of the box.

### Running Tests

WebStorm has a dedicated test runner with a visual interface showing pass/fail status for each test. Vela provides test running through:
...
Vela supports auto-detection for common test frameworks:
...
Several features that require plugins in WebStorm are built into Vela:
...
To set expectations clearly, here's what WebStorm offers that Vela doesn't have:
...
## Collaboration in Vela vs. WebStorm

WebStorm offers Code With Me as a separate feature for collaboration. Vela has collaboration built into the core experience.
...
If you're used to AI assistants in WebStorm (like GitHub Copilot, JetBrains AI Assistant, or Junie), Vela offers similar capabilities with more flexibility.
...
- [JavaScript in Vela](../languages/javascript.md) — JavaScript-specific setup and configuration
- [TypeScript in Vela](../languages/typescript.md) — TypeScript-specific setup and configuration
