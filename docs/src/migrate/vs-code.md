---
title: How to Migrate from VS Code to Vela
description: "Guide for migrating from VS Code to Vela, including settings and keybindings."
---

# How to Migrate from VS Code to Vela

This guide explains how to move from VS Code to Vela without rebuilding your workflow.
...
`brew install vela-editor/vela/vela`
...
After installation, you can launch Vela from your Applications folder (macOS) or directly from the terminal (Linux) using:
`vela .`
This opens the current directory in Vela.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. Vela imports the following settings:
...
Vela doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:

`Cmd+Shift+P → {#action vela::ImportVsCodeSettings}`
...
| VS Code | Vela | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | Vela uses Vela Mono by default |
...
Vela also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (Vela <> VS Code)
...
### Different Keybindings (Vela <> VS Code)
...
Vela also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
Vela takes a different approach.
...
- **Per-project settings are optional**: You can add a `.vela/settings.json` file inside a project to override global settings.

- **You can start from a single file or an empty window**: Vela doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, Vela encourages you to:
...
Vela does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into Vela:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. Vela's extension catalog is still growing and remains smaller.

### Collaboration in Vela vs. VS Code

Unlike VS Code, Vela doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [Vela uses Vela](https://vela.dev/blog/vela-is-our-office) to plan work and collaborate.

### Using AI in Vela

If you’re used to GitHub Copilot in VS Code, you can do the same in Vela. You can also explore other agents through Vela Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
**Custom Tasks**: Define build or run commands in your `tasks.json` (accessed via command palette: {#action vela::OpenTasks}):
...
Copy your VS Code snippet JSON directly into Vela's snippets folder ({#action snippets::ConfigureSnippets}).
