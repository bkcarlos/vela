---
title: How to Migrate from PyCharm to Vela
description: "Guide for migrating from PyCharm to Vela, including settings and keybindings."
---

# How to Migrate from PyCharm to Vela

This guide covers how to set up Vela if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.

## Install Vela

Vela is available on macOS, Windows, and Linux.

For macOS, you can download it from vela.dev/download, or install via Homebrew:

```sh
brew install --cask vela
```

For Windows, download the installer from vela.dev/download, or install via winget:

```sh
winget install Vela.Vela
```
...
After installation, you can launch Vela from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`vela .`
This opens the current directory in Vela.
...
You can configure most settings in the Settings Editor ({#kb vela::OpenSettings}). For advanced settings, run {#action vela::OpenSettingsFile} from the Command Palette to edit your settings file directly.

Settings PyCharm users typically configure first:

| Vela Setting             | What it does                                                                    |
| ----------------------- | ------------------------------------------------------------------------------- |
...
Vela also supports per-project settings. Create a `.vela/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Vela. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in Vela. The editor will treat that folder as the root of your project.

You can also launch Vela from the terminal inside any folder with:
`vela .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Vela compares to PyCharm.
...
### Different Keybindings (PyCharm → Vela)

| Action                 | PyCharm     | Vela (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
Vela doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. Vela delegates this work to language servers, which may not analyze as deeply or as broadly.
...
Vela uses the Language Server Protocol (LSP) for code intelligence. For Python, Vela provides several language servers out of the box:
...
Vela handles virtual environments through its toolchain system:

- Vela automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in Vela—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching Vela

> **Tip:** If basedpyright shows import errors for packages you've installed, check that Vela has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
Vela has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. Vela discovers environments but doesn't create them.
...
- Create a `.vela/settings.json` in your project root for project-specific settings
- Define common commands in `tasks.json` (open via Command Palette: {#action vela::OpenTasks}):
...
Vela has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from Vela's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to Vela keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). Vela uses a similar concept called "docks":

| PyCharm Tool Window | Vela Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | -------------- | --------------------------- |
...
Vela has three dock positions: left, bottom, and right. Panels can be moved between docks by dragging or through settings.

### Debugging

Both PyCharm and Vela offer integrated debugging, but the experience differs:

- Vela uses `debugpy` (the same debug adapter that VS Code uses)
...
Vela can automatically detect debuggable entry points. Press `F4` to see available options, including:

- Python scripts
- Modules
- pytest tests

For more control, create a `.vela/debug.json` file:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. Vela provides test running through:
...
Vela's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into Vela:
...
To set expectations clearly, here's what PyCharm offers that Vela doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside Vela for your Python editing
...
- **Remote interpreters** — Vela has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Collaboration in Vela vs. PyCharm

PyCharm offers Code With Me as a separate plugin for collaboration. Vela has collaboration built into the core experience.

- Open the Collab Panel in the left dock
- Create a channel and [invite your collaborators](https://vela.dev/docs/collaboration#inviting-a-collaborator) to join
- [Share your screen or your codebase](https://vela.dev/docs/collaboration#share-a-project) directly
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), Vela offers similar capabilities with more flexibility.
...
Once signed in, just start typing. Vela will offer suggestions inline for you to accept.

### Additional AI Options

To use other AI models in Vela, you have several options:

- Use Vela's hosted models, with higher rate limits. Requires [authentication](https://vela.dev/docs/authentication) and access through [Vela Pro](https://vela.dev/docs/account/vela-hosted-models.html).
- Bring your own [API keys](https://vela.dev/docs/ai/use-api-access.html), no authentication needed
- Use [External Agents like Claude Agent](https://vela.dev/docs/ai/external-agents.html)

## Advanced Config and Productivity Tweaks

Vela exposes advanced settings for power users who want to fine-tune their environment.
...
Now that you're set up, here are some resources to help you get the most out of Vela:
...
- [AI Features](../ai/overview.md) — Explore Vela's AI capabilities beyond code completion
- [Collaboration](../collaboration/overview.md) — Share your projects and code together in real time
- [Python in Vela](../languages/python.md) — Python-specific setup and configuration
