---
title: Configuring Vela - Settings and Preferences
description: Configure Vela with the Settings Editor, JSON files, and project-specific overrides. Covers all settings options.
---

# Configuring Vela

This guide explains how Vela's settings system works, including the Settings Editor, JSON configuration files, and project-specific settings.

For visual customization (themes, fonts, icons), see [Appearance](./appearance.md).

## Settings Editor

The **Settings Editor** ({#kb vela::OpenSettings}) is the primary way to configure Vela. It provides a searchable interface where you can browse available settings, see their current values, and make changes.

To open it:

- Press {#kb vela::OpenSettings}
- Or run {#action vela::OpenSettings} from the command palette
...
Your user settings apply globally across all projects. Open the file with {#kb vela::OpenSettingsFile} or run {#action vela::OpenSettingsFile} from the command palette.

The file is located at:

- macOS: `~/.config/vela/settings.json`
- Linux: `~/.config/vela/settings.json` (or `$XDG_CONFIG_HOME/vela/settings.json`)
- Windows: `%APPDATA%\Vela\settings.json`

The syntax is JSON with support for `//` comments.

### Default Settings

To see all available settings with their default values, run {#action vela::OpenDefaultSettings} from the command palette. This opens a read-only reference you can use when editing your own settings.

### Project Settings

Override user settings for a specific project by creating a `.vela/settings.json` file in your project root. Run {#action vela::OpenProjectSettings} to create this file.
...
1. **Default settings** — Vela's built-in defaults
...
Vela has some compatibility support for Emacs and Vim [modelines](./modelines.md), so you can set some settings per-file.
...
Vela supports deep links that open specific settings directly:

```
vela://settings/theme
vela://settings/vim_mode
vela://settings/buffer_font_size
```
