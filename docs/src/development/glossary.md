---
title: "Vela Development: Glossary"
description: "Guide to vela development: glossary for Vela development."
---

# Vela Development: Glossary

This page defines terms and structures used throughout the Vela codebase.
...
- Can we generate this list from doc comments throughout vela?
...
- `Window`: A struct representing a Vela window in your desktop environment (see image below). You can have multiple windows open. This is mostly passed around for rendering.
...
- `Center`: The middle of the vela window, the center is split into multiple `Pane`s. In the codebase this is a field on the `Workspace` struct. (see image below).
...
- [Multibuffer](https://vela.dev/docs/multibuffers): A list of Editors, a multi-buffer allows editing multiple files simultaneously. A multi-buffer opens when an operation in Vela returns multiple locations, examples: _search_ or _go to definition_. See project search in the image below.
...
- `Editor`: The text editor type. Most editable surfaces in Vela are an `Editor`, including single-line inputs. Each pane in the image above contains one or more `Editor` instances.
...
- `Upstream client`: The vela client which has shared their workspace
- `Downstream client`: The vela client joining a shared workspace
...
- `BreakpointStore`: Is an entity that manages breakpoint states in local and remote instances of Vela
