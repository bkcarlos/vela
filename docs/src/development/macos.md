---
title: Building Vela for macOS
description: "Guide to building vela for macos for Vela development."
---

# Building Vela for macOS
...
Vela includes visual regression tests that capture screenshots of real Vela windows and compare them against baseline images. These tests require macOS with Screen Recording permission.
...
Baseline images are stored in `crates/vela/test_fixtures/visual_tests/` but are
...
```sh
error: failed to run custom build command for gpui v0.1.0 (/Users/path/to/vela)`**
...
If Vela continually rebuilds root crates, you may be opening the Vela codebase itself in your development build.
...
build of Vela. These environment variables are in turn passed to `cargo check`, which
...
If you build Vela frequently, macOS may keep verifying new builds, which can add a few seconds to each iteration.
