---
title: Remote Development in Vela - SSH Workflows
description: Use remote development in Vela to edit code over SSH with local UI performance, remote terminals, language servers, and tasks.
---

# Remote Development

Remote Development lets you edit code on a remote server while running Vela locally. The UI stays responsive because it runs on your machine, while language servers, tasks, and terminals run on the server.
...
Remote development requires two computers, your local machine that runs the Vela UI and the remote server which runs a Vela headless server. The two communicate over SSH, so you will need to be able to SSH from your local machine into the remote server to use this feature.

![Architectural overview of Vela Remote Development](https://vela.dev/img/remote-development/diagram.png)

On your local machine, Vela runs its UI, talks to language models, uses Tree-sitter to parse and syntax-highlight code, and stores unsaved changes and recent projects. The source code, language servers, tasks, and the terminal all run on the remote server. [AI features](./ai/overview.md) work in remote sessions, including the Agent Panel and Inline Assistant.

> **Note:** The original version of remote development sent traffic via Vela's servers. As of Vela v0.157 you can no longer use that mode.

## Setup

1. Download and install the latest [Vela](https://vela.dev/releases). You need at least Vela v0.159.
...
1. Your local machine will attempt to connect to the remote server using the `ssh` binary on your path. Assuming the connection is successful, Vela will download the server on the remote host and start it.
1. Once the Vela server is running, you will be prompted to choose a path to open on the remote server.
   > **Note:** Vela does not currently handle opening very large directories (for example, `/` or `~` that may have >100,000 files) very well. We are working on improving this, but suggest in the meantime opening only specific projects, or subfolders of very large mono-repos.

For simple cases where you don't need any SSH arguments, you can run `vela ssh://[<user>@]<host>[:<port>]/<path>` to open a remote folder/file directly. The CLI also accepts the scp style `vela ssh://[<user>@]<host>:~/project` or `vela ssh://[<user>@]<host>:/absolute/path`. If you'd like to hotlink into an SSH project, use a link of the format: `vela://ssh/[<user>@]<host>[:<port>]/<path>`.

## Supported platforms

The remote machine must be able to run Vela's server. The following platforms should work, though note that we have not exhaustively tested every Linux distribution:
...
The list of remote servers is stored in your settings file {#kb vela::OpenSettings}. You can edit this list using the Remote Projects dialog {#kb projects::OpenRemote}, which provides some robustness - for example it checks that the connection can be established before writing it to the settings file.
...
Vela shells out to the `ssh` on your path, and so it will inherit any configuration you have in `~/.ssh/config` for the given host. That said, if you need to override anything you can configure the following additional options on each connection:
...
There are two additional Vela-specific options per connection, `upload_binary_over_ssh` and `nickname`:
...
If you use the command line to open a connection to a host by doing `vela ssh://192.168.1.10/~/.vimrc`, then extra options are read from your settings file by finding the first connection that matches the host/username/port of the URL on the command line.

Additionally it's worth noting that while you can pass a password on the command line `vela ssh://user:password@host/~`, we do not support writing a password to your settings file. If you're connecting repeatedly to the same host, you should configure key-based authentication.

## Remote Development on Windows (SSH)

Vela on Windows supports SSH remoting and will prompt for credentials when needed.
...
Vela supports opening folders inside of WSL natively on Windows.
...
- The local Vela settings (in `~/.vela/settings.json` on macOS or `~/.config/vela/settings.json` on Linux) on your local machine.
- The server Vela settings (in the same place) on the remote server.
- The project settings (in `.vela/settings.json` or `.editorconfig` of your project)

Both the local Vela and the server Vela read the project settings, but they are not aware of the other's main settings file.
...
In most cases, your remote server will already have proxy environment variables configured. Vela will automatically use them when downloading language servers, communicating with LLM models, etc.
...
Alternatively, you can configure the proxy in the remote machine's `~/.config/vela/settings.json` (Linux) or `~/.vela/settings.json` (macOS):
...
Once you provide the SSH options, Vela shells out to `ssh` on your local machine to create a ControlMaster connection with the options you provide.
...
Stripping debug symbols matches the packaging step in [`script/bundle-linux`](https://github.com/vela-industries/vela/blob/main/script/bundle-linux) and keeps the binary closer to the size of Vela's prebuilt downloads. Prefer `llvm-objcopy` because GNU `objcopy` may not understand newer LLVM-emitted CREL sections.

The official Linux release builds a musl/static remote server. If you need that, follow the current target and flags in [`script/bundle-linux`](https://github.com/vela-industries/vela/blob/main/script/bundle-linux); for x86-64 Linux, that looks like:
...
If you do this, you must upload it to `~/.vela_server/vela-remote-server-{RELEASE_CHANNEL}-{VERSION}` on the server, for example `~/.vela_server/vela-remote-server-stable-0.217.3+stable.105.80433cb239e868271457ac376673a5f75bc4adb1`. The version must exactly match the version of Vela itself you are using.

## Maintaining the SSH connection

Once the server is initialized, Vela will create new SSH connections (reusing the existing ControlMaster) to run the remote development server.
...
In the case that reconnecting fails, the daemon will not be re-used. That said, unsaved changes are by default persisted locally, so that you do not lose work. You can always reconnect to the project at a later date and Vela will restore unsaved changes.

If you are struggling with connection issues, you should be able to see more information in the Vela log `cmd-shift-p Open Log`. If you are seeing things that are unexpected, please file a [GitHub issue](https://github.com/vela-industries/vela/issues/new) or reach out in the #support forums on [Discord](https://vela.dev/community-links).

## Supported SSH Options

Under the hood, Vela shells out to the `ssh` binary to connect to the remote server. We create one SSH control master per project, and then use that to multiplex SSH connections for the Vela protocol itself, any terminals you open and tasks you run. We read settings from your SSH config file, but if you want to specify additional options to the SSH control master you can configure Vela to set them.
...
Note that we deliberately disallow some options (for example `-t` or `-T`) that Vela will set for you.

## Known Limitations

- You can't open files from the remote Terminal by typing the `vela` command.
...
- [Git Worktrees](./git.md#git-worktrees): Create and switch between linked
  Git worktrees. Vela supports the worktree picker in remote projects when the
  remote connection is active.
- [Configuring Vela](./configuring-vela.md): Manage shared and project settings,
  including `.vela/settings.json`.
- [Agent Panel](./ai/agent-panel.md): Use AI workflows in remote projects.
- [Remote Development on vela.dev](https://vela.dev/remote-development): Product
