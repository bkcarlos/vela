...
To restore a thread, open Thread History and click the thread you want to bring back. Vela moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Vela restores the worktree automatically.
...
Each thread can use a different agent, so you can run Vela's built-in agent in one thread and an [External Agent](./external-agents.md) like Claude Code or Codex in another.
...
| --------------------------------------------- | ------------------------------------------------------------------------------- |
| [Vela Agent thread](./vela-agent.md)            | Uses Vela Agent settings, profiles, tools, Skills, Instructions, and MCP         |
...
The Threads Sidebar can hold multiple projects at once. Each project gets its own group with its own threads and conversation history. This mirrors how Vela handles projects in general — see [Windows & Projects](../windows-and-projects.md) for more on how projects open and how to manage them.
...
To automate setup steps whenever a new worktree is created, use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Vela creates a linked worktree, with `VELA_WORKTREE_ROOT` pointing at the new worktree and `VELA_MAIN_GIT_WORKTREE` pointing at the original repository.
...
- [Terminal Threads](./terminal-threads.md): Run agent CLIs and TUIs directly in Vela
