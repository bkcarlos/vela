---
title: Use a Gateway - Vela
description: Configure OpenRouter, Vercel AI Gateway, Amazon Bedrock, and other gateway or cloud model platforms in Vela.
---
...
| Gateway                   | Vela AI features | External Agents | Terminal Threads | Notes                                        |
| ------------------------- | --------------- | --------------- | ---------------- | -------------------------------------------- |
...
Use OpenRouter when you want to route Vela AI features through OpenRouter.
...
Vela also reads `OPENROUTER_API_KEY` from the local Vela process environment.
...
Use Vercel AI Gateway when you want to route Vela AI features through Vercel.
...
Vela also reads `VERCEL_AI_GATEWAY_API_KEY` from the local Vela process environment.
...
Bedrock supports Vela-prefixed AWS environment variables so Vela does not override or consume your normal AWS credentials:

- `VELA_ACCESS_KEY_ID`
- `VELA_SECRET_ACCESS_KEY`
- `VELA_SESSION_TOKEN`
- `VELA_AWS_PROFILE`
- `VELA_AWS_REGION`
- `VELA_AWS_ENDPOINT`
- `VELA_BEDROCK_BEARER_TOKEN`
...
Vela uses [Cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) for Bedrock on a best-effort basis.

By default, Vela uses regional inference profiles. To opt into global profiles, add `allow_global`:
...
`protocol` selects which OpenAI-compatible API the model is called through, and must be either `chat_completions` or `responses`. Set `supports_thinking` to `true` for custom Mantle models that accept OpenAI reasoning effort parameters; Vela will then expose `low`, `medium`, `high`, and `xhigh` in the thinking effort picker, while disabling thinking sends `none`.
