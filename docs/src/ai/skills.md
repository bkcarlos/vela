---
title: Agent Skills - Vela
description: Extend Vela's AI agent with reusable, on-demand skill files for specialized tasks.
---
...
Vela includes a built-in `create-skill` skill — invoke it with `/create-skill` and the agent walks you through the process.
...
Lastly, it's also possible to add a skill through importing it from an existing GitHub Markdown file. Open the command palette and look for the {#action agent::CreateSkillFromUrl} action. If your clipboard contains a supported GitHub `.md` URL, Vela pre-fills and fetches it automatically.
...
Open the Settings Editor (`Cmd+,` on macOS, `Ctrl+,` on Linux/Windows) and navigate to **AI > Skills**, or go directly to [agent.skills](vela://settings/agent.skills).

The **User** tab shows your global skills, and each **Project** tab shows the skills for that project.

For each skill you can:

- **Copy Share Link** — copies a `vela://skill` link that embeds the skill, ready to send to someone else (see [Sharing Skills](#sharing-skills))
...
You can hand a skill to a teammate without hosting it anywhere. In the Skills settings page, click the **link** icon on a skill row to copy a `vela://skill?data=…` link to your clipboard.
...
When someone opens that link (for example by pasting it into their browser or clicking it in a chat), Vela opens the "Create Skill" page in the settings window, pre-filled with the shared skill.
...
When the agent invokes a skill you created or installed, Vela prompts you to allow or deny it, using the same permission flow as other tools. Skills built into Vela do not prompt. You can set per-skill defaults in [Tool Permissions](./tool-permissions.md) so you're not prompted for skills you always trust.
...
Vela loads skills from two locations:
...
Vela Skills apply to the Vela Agent. External Agents and Terminal Threads may have their own native skills, prompts, or instruction systems. Configure those in the External Agent or CLI.
