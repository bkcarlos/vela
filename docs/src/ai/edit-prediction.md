---
title: AI Code Completion in Vela - Zeta, Copilot, Codestral, Mercury Coder
description: Set up AI code completions in Vela with Zeta (built-in), GitHub Copilot, Codestral, or Mercury Coder. Multi-line predictions as you type.
---

# Edit Prediction

Edit Prediction is how Vela's AI code completions work: an LLM predicts the code you want to write.
As you type, Vela requests predictions from the edit prediction provider, which returns individual or multi-line suggestions you accept by pressing `tab`.

The default provider is [Zeta, an open source model developed by Vela](https://vela.dev/blog/zeta2), but you can also use [other providers](#other-providers) like GitHub Copilot, Mercury Coder, and Codestral.
...
You can confirm that Zeta is properly configured by opening the [Settings Editor](vela://settings/edit_predictions.providers) (`Cmd+,` on macOS or `Ctrl+,` on Linux/Windows) and searching for `edit_predictions`. The `provider` field should be set to `Vela AI`.
...
The free plan includes 2,000 Zeta predictions per month. The [Pro plan](../account/plans-and-pricing.md) removes this limit. See [Vela's pricing page](https://vela.dev/pricing) for details.
...
![Edit Prediction status bar menu, with the modes toggle.](https://vela.dev/img/edit-prediction/status-bar-menu.webp)
...
Open the keymap editor with {#action vela::OpenKeymap} ({#kb vela::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and hit `edit`. Then change the context the binding is active in to just `Editor && edit_prediction` and save it.
...
Open the keymap editor with {#action vela::OpenKeymap} ({#kb vela::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and delete it.
...
Open the keymap editor with {#action vela::OpenKeymap} ({#kb vela::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and delete it. Then right click on the binding for `alt-tab`, select "Edit", and record your desired keystrokes before saving.
...
If you configured edit prediction keybindings before Vela `v0.229.0`, your `keymap.json` may have entries that are now redundant.
...
**Renamed context**: The `edit_prediction_conflict` context has been replaced by `edit_prediction && (showing_completions || in_leading_whitespace)`. Vela automatically migrates any bindings that used `edit_prediction_conflict`, so no changes are required on your end.
...
To not have predictions appear automatically as you type, set this in your settings file ([how to edit](../configuring-vela.md#settings-files)):
...
To not have predictions appear automatically as you type when working with a specific language, set this in your settings file ([how to edit](../configuring-vela.md#settings-files)):
...
To disable edit predictions for specific directories or files, set this in your settings file ([how to edit](../configuring-vela.md#settings-files)):

```json [settings]
{
  "edit_predictions": {
    "disabled_globs": ["~/.config/vela/settings.json"]
  }
}
```
...
To use GitHub Copilot as your provider, set this in your settings file ([how to edit](../configuring-vela.md#settings-files)):
...
If your organization uses GitHub Copilot Enterprise, you can configure Vela to use your enterprise instance by specifying the enterprise URI in your settings file ([how to edit](../configuring-vela.md#settings-files)):
...
Once set, Vela routes Copilot requests through your enterprise endpoint.
...
1. Open the Settings Editor ({#kb vela::OpenSettings})
...
With `"prompt_format": "infer"`, Vela automatically uses Zeta 2 format for models named `zeta2` and Zeta 2.1 format for models named `zeta2.1`.
