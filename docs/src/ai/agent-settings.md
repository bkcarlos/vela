---
title: Agent Settings - Vela
description: Map the AI settings pages to Vela AI setup for LLM providers, External Agents, MCP servers, and related settings.
---
...
You can also reach the same page with {#action vela::OpenSettings} and selecting **AI** in the sidebar.
...
| Settings file   | {#action vela::OpenSettingsFile} | Direct JSON edits and settings not exposed in UI                            |

For general settings mechanics, see [Configuring Vela](../configuring-vela.md).

## LLM Providers {#llm-providers}

The `LLM Providers` section configures model providers for Vela AI features, including Vela Agent, Inline Assistant, Git commit generation, thread summaries, and similar model-backed features.
...
Some Vela AI features have their own model or prompt settings in `settings.json`, including:
...
Vela Agent can automatically compact long threads before they reach the selected model's context window. Compaction summarizes earlier messages and keeps the conversation usable without starting a new thread.
...
`0` is not a valid threshold. If the threshold is invalid, Vela falls back to `90%`.

You can compact a Vela Agent thread manually at any time by typing `/compact` in the Agent Panel message editor. For more on thread token usage and compaction behavior, see [Token Usage and Compaction](./agent-panel.md#token-usage).
...
The `MCP Servers` page configures Model Context Protocol servers connected to Vela.
...
| Edit raw settings JSON                                       | [Configuring Vela](../configuring-vela.md)       |

## Feature-Specific Models {#feature-specific-models}

Vela supports feature-specific model settings for Inline Assistant, Git commit generation, thread summaries, and subagents. Configure these in settings when you need a different model for a specific workflow.
...
Most Vela AI features use the selected model's default generation behavior.
...
Vela checks matching entries from last to first. An entry can omit `provider` or
