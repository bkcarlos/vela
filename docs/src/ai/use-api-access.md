---
title: Use API Access - Vela
description: Configure provider API access, API keys, API credits, usage billing, and OpenAI-compatible endpoints for Vela AI features.
---
...
Vela supports these first-class API providers for model-backed Vela AI features:
...
Use API access for the Vela Agent, Inline Assistant, Git commit generation,
thread summaries, and similar Vela-owned AI features.
...
Most API-access providers can be configured on the **Settings → AI → LLM Providers** page with {#action agent::OpenSettings}. Keys saved through Vela are stored in the system keychain, not in `settings.json`.

Vela also reads provider-specific environment variables. Non-empty environment variables take precedence over keychain values. If a key comes from an environment variable, unset the variable and restart Vela to stop using it.
...
You can attach extra HTTP headers to every request Vela makes to supported HTTP-based providers. This is useful in corporate environments or for observability tooling.
...
Headers managed by Vela for each provider, such as `Authorization`, `Content-Type`, `Accept`, and provider-specific authentication headers, are ignored with a warning if you try to override them.

## Remote Projects {#remote-projects}

Vela LLM providers for Vela AI features are initialized in the local Vela app. In SSH, dev container, and other remote projects, API keys saved in Vela are read from the local system keychain, and provider environment variables are read from the local Vela process environment.
...
Vela also reads `ANTHROPIC_API_KEY` from the local Vela process environment.
...
Vela also reads `OPENAI_API_KEY` from the local Vela process environment.
...
Vela reads `GEMINI_API_KEY`, falling back to `GOOGLE_AI_API_KEY`, from the local Vela process environment.
...
Vela also reads `MISTRAL_API_KEY` from the local Vela process environment.
...
Use DeepSeek API access when you have paid API usage, top-ups, or an API key. In Vela, DeepSeek is API access, not subscription sign-in.
...
Vela also reads `DEEPSEEK_API_KEY` from the local Vela process environment.
...
Vela also reads `XAI_API_KEY` from the local Vela process environment.
...
Vela does not sign in to OpenCode with OAuth or detect your OpenCode subscription; it uses an OpenCode API key saved in the system keychain or `OPENCODE_API_KEY`.
...
Vela also reads `OPENCODE_API_KEY` from the local Vela process environment.
...
**Note:** Vela only bundles configuration for long-term OpenCode Free models. Free models that are available for a limited time are not included in Vela. To use those models, add a custom OpenCode model with configuration from [the OpenCode website](https://opencode.ai/docs/zen#pricing) and [models.dev](https://github.com/anomalyco/models.dev/tree/dev/providers/opencode/models).

#### Custom OpenCode Models {#opencode-custom-models}

The Vela Agent comes preconfigured with OpenCode models. Add custom OpenCode models when you need newer models, limited-time Free models, or models with custom endpoints.
...
The optional `custom_headers` map adds extra headers to every request, which some providers require. Headers managed by Vela (such as `X-Api-Key` and `Anthropic-Version`) cannot be overridden.
...
If a model only works with the Responses API, set `capabilities.chat_completions` to `false`. Vela will use the Responses endpoint for that model.

For reasoning models (e.g. GPT-5), set `reasoning_effort` to the non-`none` effort level your endpoint supports. This enables thinking in the agent panel and tells Vela which effort to send when thinking is enabled. The provider settings UI can configure this when adding an OpenAI-compatible provider. Vela sends OpenAI-style `reasoning_effort` on chat-completions requests.
