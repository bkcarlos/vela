...
> 估算基线：Vela `6297c88f42`、Pi `9b3a2059`  
> 估算精度：早期 ROM（量级估算），误差约 `-25% / +40%`

## 1. 结论

在**直接 fork Vela、AI Runtime 全部使用进程内 Rust、macOS arm64 首发**的前提下：
...
- 1 人：Vela/GPUI、编辑器、Diff、文件树；
- 1 人：Rust Agent、模型供应商、上下文和权限；
- 1 人：配置、Git、语言环境、自动化测试与发布。

如果开发者此前没有维护过 Vela/GPUI，大约再增加 **20%–35%**。兼职开发则不能按人数线性折算。

## 2. 为什么不是从零开发

以下能力 Vela 已经具备，可直接复用：
...
因此项目不是“开发一个新 IDE”，而是“维护一个有较深定制的 Vela fork”。主要成本来自把现有能力连接成要求的产品流程，以及长期跟进 Vela 上游。
...
| 模块 | 乐观 | 常规 | 悲观 | 说明 |
|---|---:|---:|---:|---|
| Vela 构建、fork、品牌和开发脚本 | 1 | 2 | 3 | 首次熟悉 workspace 成本较高 |
| Vela Agent 架构梳理与进程内内核调整 | 2 | 4 | 7 | 不使用 Pi 子进程会增加 Rust 实现量 |
...
| P0 语言环境与 fixture | 2 | 4 | 6 | TS/Python/Go/Rust，主体复用 Vela |
| 会话恢复、迁移和异常隔离 | 2 | 4 | 7 | 包括未处理 ChangeSet 恢复 |
| 自动化测试、性能与 soak test | 4 | 7 | 11 | 稳定 Beta 的主要成本之一 |
| macOS 签名、打包、升级与许可证 | 2 | 4 | 7 | 正式分发前必须完成 |

模块直接相加约为 **38–108 人周**，常规值约 **68 人周**。技术验证后可确认哪些 Vela 能力能直接复用，从而删除部分工作；多人并行可以缩短日历时间，但不会等比例减少总人周。稳定 Beta 建议按 **55–75 有效人周（已含主要集成缓冲）**规划。
...
可压缩项：TOML 先覆盖 CodeIDE 新配置，不一次性映射 Vela 全部设置。
...
这是当前需求中最容易被低估的模块。Vela 已有成熟的 JSON/JSONC Settings 体系。如果要求所有现有 Vela 设置都通过新 UI 和 TOML 双向映射，需要处理 schema、默认值、来源、语言覆盖、热加载、注释保留和迁移。

建议 Alpha 只让 CodeIDE 新能力使用 TOML；Beta 再逐步覆盖高频 Vela 设置。这样可以节省首版约 **3–6 周**。
...
不使用子进程能获得更自然的 Vela/LSP 集成，但 Agent 卡死、panic、超大工具结果和取消失败会直接影响 IDE。必须投入任务隔离、超时、内存边界和异常测试。
...
1. 只支持 macOS arm64；
2. 直接复用 Vela 文件树和大部分 Git UI；
3. TOML 先管理 CodeIDE 新设置，不替换全部 Vela 设置；
...
7. P0 保留 TS/Python/Go/Rust，其他语言沿用 Vela 原有能力但不承诺专项验收。
