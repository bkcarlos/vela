...
```markdown
![Alt text description](https://vela.dev/img/path/to/image.webp)
```
...
- **Practical over promotional**: Focus on what users can do, not on selling Vela. Avoid marketing language like "powerful," "revolutionary," or "best-in-class."
- **Honest about limitations**: When Vela lacks a feature or doesn't match another tool's depth, say so directly. Pair limitations with workarounds or alternative workflows.
...
- **Present tense**: "Vela opens the file" not "Vela will open the file."
...
```
Vela provides a powerful and seamless formatting experience. Simply navigate to the settings and you'll find the format_on_save option which enables Vela's incredible auto-formatting capabilities.
```

### Good: Honest about limitations

```
Vela doesn't index your project like IntelliJ does. You open a folder and start working immediately—no waiting. The trade-off: cross-project analysis relies on language servers, which may not go as deep.
...
```
While some users might miss indexing, Vela's approach is actually better because it's faster.
```
...
## Vela-Specific Conventions

### Recognized Rules Files

When documenting rules/instructions for AI, note that Vela recognizes these files (in priority order):
...
- Linux: `~/.config/vela/settings.json`
- Windows: `%AppData%\Vela\settings.json`

### Keymap File Locations

- macOS: `~/.config/vela/keymap.json`
- Linux: `~/.config/vela/keymap.json`
- Windows: `%AppData%\Vela\keymap.json`
