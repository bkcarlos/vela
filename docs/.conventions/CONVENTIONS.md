# Vela Documentation Conventions

This document covers structural conventions for Vela documentation: what to document, how to organize it, and when to create new pages.
...
- **Version-specific behavior changes** — Include version callouts (e.g., "In Vela v0.224.0 and above...")
...
- Users would search for it **by name** (e.g., "Vela terminal", "Vela snippets")
...
- Commands: `:w`, `:q`
- File paths: `~/.config/vela/settings.json`
- Action names: `git::Commit`
- Values: `true`, `false`, `"eager"`

### Action and Keybinding References

Use Vela's special syntax for dynamic rendering:
...
```markdown
> **Note:** In Vela v0.224.0 and above, tool approval is controlled by `agent.tool_permissions.default`.
```
...
- Link to `vela.dev` pages when appropriate
...
- For main feature pages with a matching marketing page, include a relevant
  `vela.dev` marketing link in addition to docs links
...
Keep language docs focused on Vela-specific configuration, not general language tutorials.
...
> Configure inline blame in Settings ({#kb vela::OpenSettings}) by searching for "inline blame", or add to your settings file:
>
> ```json [settings]
> {
>   "git": {
...
> Add the following to your settings file ([how to edit](./configuring-vela.md#settings-files)):

### Settings File Locations

- **macOS/Linux:** `~/.config/vela/settings.json`
...
- [ ] Main feature pages include a relevant `vela.dev` marketing link
