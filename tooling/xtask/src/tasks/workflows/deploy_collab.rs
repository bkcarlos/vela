    ...
    fn build_docker_image() -> Step<Run> {
        named::bash(indoc! {r#"
            docker build -f Dockerfile-collab \
              ...
              --tag "registry.digitalocean.com/vela/collab:$GITHUB_SHA" \
              .
        "#})
    }

    fn publish_docker_image() -> Step<Run> {
        named::bash(r#"docker push "registry.digitalocean.com/vela/collab:${GITHUB_SHA}""#)
    }
    ...
    fn start_rollout() -> Step<Run> {
        named::bash(indoc! {r#"
            ...
            if [[ $GITHUB_REF_NAME = "collab-production" ]]; then
              export VELA_KUBE_NAMESPACE=production
              export VELA_COLLAB_LOAD_BALANCER_SIZE_UNIT=10
              export VELA_API_LOAD_BALANCER_SIZE_UNIT=2
            elif [[ $GITHUB_REF_NAME = "collab-staging" ]]; then
              export VELA_KUBE_NAMESPACE=staging
              export VELA_COLLAB_LOAD_BALANCER_SIZE_UNIT=1
              export VELA_API_LOAD_BALANCER_SIZE_UNIT=1
            else
              echo "cowardly refusing to deploy from an unknown branch"
              exit 1
            fi

            echo "Deploying collab:$GITHUB_SHA to $VELA_KUBE_NAMESPACE"

            source script/lib/deploy-helpers.sh
            export_vars_for_environment "$VELA_KUBE_NAMESPACE"

            VELA_DO_CERTIFICATE_ID="$(doctl compute certificate list --format ID --no-header)"
            export VELA_DO_CERTIFICATE_ID
            export VELA_IMAGE_ID="registry.digitalocean.com/vela/collab:${GITHUB_SHA}"

            export VELA_SERVICE_NAME=collab
            export VELA_LOAD_BALANCER_SIZE_UNIT=$VELA_COLLAB_LOAD_BALANCER_SIZE_UNIT
            export DATABASE_MAX_CONNECTIONS=850
            envsubst < crates/collab/k8s/collab.template.yml | kubectl apply -f -
            kubectl -n "$VELA_KUBE_NAMESPACE" rollout status "deployment/$VELA_SERVICE_NAME" --watch
            echo "deployed ${VELA_SERVICE_NAME} to ${VELA_KUBE_NAMESPACE}"

            export VELA_SERVICE_NAME=api
            export VELA_LOAD_BALANCER_SIZE_UNIT=$VELA_API_LOAD_BALANCER_SIZE_UNIT
