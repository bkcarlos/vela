...
pub(crate) const VELA_EXTENSION_CLI_SHA: &str = "9ee3c503a4bbbc6b4a0f8a789acca4871d773223";
...
// This is used by various extensions repos in the vela-extensions org to run automated tests.
pub(crate) fn extension_tests() -> Workflow {
    ...
    named::workflow()
        ...
        .add_env(("VELA_EXTENSION_CLI_SHA", VELA_EXTENSION_CLI_SHA))
...
pub(crate) fn check_extension() -> NamedJob {
    let (cache_download, cache_hit) = cache_vela_extension_cli();
    ...
    let job = Job::default()
        ...
        .add_step(download_vela_extension_cli(cache_hit))
...
pub fn cache_vela_extension_cli() -> (Step<Use>, StepOutput) {
    let step = named::uses(
        "actions",
        "cache",
        "0057852bfaa89a56745cba8c7296529d2fc39830",
    )
    .id("cache-vela-extension-cli")
    .with(
        Input::default()
            .add("path", "vela-extension")
            .add("key", "vela-extension-${{ env.VELA_EXTENSION_CLI_SHA }}"),
    );
    let output = StepOutput::new(&step, "cache-hit");
    (step, output)
}

pub fn download_vela_extension_cli(cache_hit: StepOutput) -> Step<Run> {
    named::bash(
    indoc! {
        r#"
        wget --quiet "https://vela-extension-cli.nyc3.digitaloceanspaces.com/$VELA_EXTENSION_CLI_SHA/x86_64-unknown-linux-gnu/vela-extension" -O "$GITHUB_WORKSPACE/vela-extension"
        chmod +x "$GITHUB_WORKSPACE/vela-extension"
        "#,
    }
...
pub fn check() -> Step<Run> {
    named::bash(indoc! {
        ...
        "$GITHUB_WORKSPACE/vela-extension" --source-dir . --scratch-dir /tmp/ext-scratch --output-dir /tmp/ext-output
        "#
    })
}
...
fn verify_version_did_not_change(version_changed: StepOutput) -> Step<Run> {
    named::bash(indoc! {r#"
        if [[ "$VERSION_CHANGED" == "true" && "$GITHUB_EVENT_NAME" == "pull_request" && "$PR_USER_LOGIN" != "vela-zippy[bot]" ]] ; then
            echo "Version change detected in your change!"
            echo "Version changes happen in separate PRs and will be performed by the vela-zippy bot"
            exit 42
        fi
        "#
    })
