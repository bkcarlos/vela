...
use vela_actions::{ChangeKeybinding, OpenKeymap};
...
impl KeymapEditor {
    ...
    fn process_bindings(
        ...
        vela_keybind_context_language: Arc<Language>,
        humanized_action_names: &HumanizedActionNameCache,
        cx: &mut App,
    ) -> (
            ...
            let context = key_binding
                ...
                .map(|predicate| {
                    KeybindContextString::Local(
                        ...
                        vela_keybind_context_language.clone(),
                    )
                })
    ...
    fn on_keymap_changed(&mut self, window: &mut Window, cx: &mut Context<KeymapEditor>) {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let vela_keybind_context_language =
    ...
    fn toggle_vela_defaults_filter(&mut self, cx: &mut Context<Self>) {
        self.source_filters.vela_defaults = !self.source_filters.vela_defaults;
        self.on_query_changed(cx);
    }
                ...
                Some(ContextMenu::build_persistent(window, cx, {
                    ...
                    move |mut menu, _window, cx| {
                        ...
                        menu = menu
                            ...
                            .map(add_filter(
                                "Default",
                                source_filters.vela_defaults,
                                None,
                                &focus_handle,
                                &keymap_editor,
                                Some(|editor, cx| {
                                    editor.toggle_vela_defaults_filter(cx);
                                }),
                            ))
                            .map(add_filter(
                                "Vim",
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "Edit in JSON")
                                            .key_binding(
                                                ui::KeyBinding::for_action_in(&vela_actions::OpenKeymapFile, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10.))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                                                    vela_actions::OpenKeymapFile.boxed_clone(),
                                                    cx,
                                                );
                                            })
                                    )
...
async fn load_keybind_context_language(
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("Vela Keybind Context")
        })
        .context("Failed to load Vela Keybind Context language")
        .log_err();
    ...
    language.unwrap_or_else(|| {
        Arc::new(Language::new(
            LanguageConfig {
                name: "Vela Keybind Context".into(),
                ..Default::default()
            },
...
mod tests {
    ...
    {
        "bindings": {
            "alt-cmd-shift-c": "vela::OpenKeymap"
        }
    },
    {
        "bindings": {
        ...
        let rows = keymap_editor.read_with(cx, |editor, _| {
            visible_rows_for_action(editor, "vela::OpenKeymap")
        });
        assert_eq!(
            rows.len(),
            2,
    ...
    fn binding_is_unbound_by_unbind_respects_precedence() {
        let binding = gpui::KeyBinding::new("tab", vela_actions::OpenKeymap, None);
        let unbind =
