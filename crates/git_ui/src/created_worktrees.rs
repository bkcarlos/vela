//! A persistent registry of git worktrees that Vela itself created.
//!
//! Thread archival deletes a worktree's directory from disk, so it must be
//! certain the worktree was created by Vela rather than by the user. This
//! module records each Vela-created worktree in the local database, keyed by
//! its path (and remote host, for remote projects), along with the creation
//! time of the worktree's git metadata directory at the time Vela created it.
//!
...
//! recreated outside Vela, so deletion must be skipped. Every failure mode
...
//! different Vela install (e.g. another release channel, or another machine
...
/// Records that Vela created the worktree at `worktree_path`, along with the
...
/// Returns the recorded creation time for a worktree Vela created, or `None`
/// if Vela has no record of creating it.
...
/// Removes the record for a worktree, either because Vela deleted it or
/// because the directory on disk turned out not to be the one Vela created.
