...
fn register_web_search_providers(
    ...
    register_vela_web_search_provider(
        registry,
        client.clone(),
        user_store.clone(),
...
fn register_vela_web_search_provider(
    registry: &mut WebSearchRegistry,
    client: Arc<Client>,
    user_store: Entity<UserStore>,
    ...
    let using_vela_provider = language_model_registry
        .read(cx)
        .default_model()
        .is_some_and(|default| default.is_provided_by_vela());
    if using_vela_provider {
        ...
        registry.unregister_provider(WebSearchProviderId(
            cloud::VELA_WEB_SEARCH_PROVIDER_ID.into(),
        ));
    }
}
