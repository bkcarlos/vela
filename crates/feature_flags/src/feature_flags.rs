...
pub static VELA_DISABLE_STAFF: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("VELA_DISABLE_STAFF").is_ok_and(|value| !value.is_empty() && value != "0")
});
...
///
/// Feature flags are enabled for members of Vela staff by default. To disable this behavior
/// so you can test flags being disabled, set VELA_DISABLE_STAFF=1 in your environment,
/// which will force Vela to treat the current user as non-staff.
pub trait FeatureFlag {
    ...
    /// Returns whether this feature flag is enabled for Vela staff.
