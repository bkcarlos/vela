...
pub const KERNEL_DOCS_URL: &str = "https://vela.dev/docs/repl#changing-kernels";

pub fn init(fs: Arc<dyn Fs>, cx: &mut App) {
    set_dispatcher(vela_dispatcher(cx));
    repl_sessions_ui::init(cx);
    ReplStore::init(fs, cx);
}

fn vela_dispatcher(cx: &mut App) -> impl Dispatcher {
    struct VelaDispatcher {
    ...
    // other crates in Vela.
    impl Dispatcher for VelaDispatcher {
        #[track_caller]
