...
impl PythonDebugAdapter {
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in Vela terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_vela_format(
        ...
        let mut args = json!({
            "request": match vela_scenario.request {
                ...
                },
                {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "envFile": {
                                "default": "${VELA_WORKTREE_ROOT}/.env",
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because Vela defaults to the cwd worktree root)
