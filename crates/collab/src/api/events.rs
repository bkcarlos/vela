...
pub struct VelaChecksumHeader(Vec<u8>);

impl Header for VelaChecksumHeader {
    fn name() -> &'static HeaderName {
        static VELA_CHECKSUM_HEADER: OnceLock<HeaderName> = OnceLock::new();
        VELA_CHECKSUM_HEADER.get_or_init(|| HeaderName::from_static("x-vela-checksum"))
    }
...
pub async fn post_events(
    ...
    TypedHeader(VelaChecksumHeader(checksum)): TypedHeader<VelaChecksumHeader>,
...
pub fn calculate_json_checksum(app: Arc<AppState>, json: &impl AsRef<[u8]>) -> Option<Vec<u8>> {
    let checksum_seed = app.config.vela_client_checksum_seed.as_ref()?;
...
fn for_snowflake(
    ...
    body.events.into_iter().map(move |event| {
        ...
        // dictionary. See https://app.amplitude.com/data/vela/Vela/sources/detail/production/falcon%3A159998
