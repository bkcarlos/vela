...
impl LmStudioLanguageModel {
    fn to_lmstudio_request(
        ...
        Ok(lmstudio::ChatCompletionRequest {
            ...
            // It would be a bad UX to silently override these settings from Vela, so we pass no temperature as a default.
...
impl ConfigurationView {
    ...
    fn render_api_key_editor(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "You can also set the {API_KEY_ENV_VAR_NAME} environment variable and restart Vela."
                ))
