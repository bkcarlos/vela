//! A module, responsible for managing the trust logic in Vela.
//!
...
//!
//! Vela does not consider invisible, `worktree.is_visible() == false` worktrees in Vela, as those are programmatically created inside Vela for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! Vela has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty Vela it's possible to open just a file, same as after opening a directory in Vela it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening Vela's settings.json file via `vela: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and Vela needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in Vela, it's a full worktree which may spawn multiple language servers associated with it.
...
//! * "path override"
//!
//! To ease trusting multiple directory worktrees at once, it's possible to trust a parent directory of a certain directory worktree opened in Vela.
...
impl TrustedWorktreesStore {
    ...
    /// Requires Vela's restart to take proper effect.
    ...
    pub fn can_trust(
        ...
        // Vela opened an "internal" directory: e.g. a tmp dir for `keymap_editor.rs` needs.
