mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${VELA_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$VELA_FILE'".into(),
            command: "$VELA_CUSTOM_PYTHON_ACTIVE_VELA_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$VELA_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$VELA_WORKTREE_ROOT".into()),
            use_new_terminal: false,
            allow_concurrent_runs: false,
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$VELA_CUSTOM_PYTHON_ACTIVE_VELA_TOOLCHAIN",
                "args": [],
                "cwd": "$VELA_WORKTREE_ROOT",
                "module": "$VELA_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
