use super::register_vela_scheme;
use anyhow::Result;
...
actions!(
    cli,
    [
        /// Installs the Vela CLI tool to the system PATH.
        InstallCliBinary,
    ]
);

const CANT_INSTALL_DOCS_URL: &str = "https://vela.dev/docs/macos#cant-install-cli";
...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    ...
    let link_path = Path::new("/usr/local/bin/vela");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed Vela from our official release add ~/.local/bin to your PATH.\n\nIf you installed Vela from a different source like your package manager, then you may need to create an alias/symlink manually.\n\nDepending on your package manager, the CLI might be named velaitor, velait, vela-editor or something else.";
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        let path = match install_script(cx.deref()).await {
            ...
            Ok(None) => return Ok(()),
            Err(error) => {
                log::error!("failed to install vela CLI: {error:#}");
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `vela` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the Vela CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            struct InstalledVelaCli;

            workspace.show_toast(
                Toast::new(
                    NotificationId::unique::<InstalledVelaCli>(),
                    format!(
                        "Installed `vela` to {}. You can launch {} from your terminal.",
        ...
        register_vela_scheme(cx).await.log_err();
        Ok(())
    })
    .detach_and_prompt_err("Cannot install the Vela CLI", window, cx, |_, _, _| None);
}
