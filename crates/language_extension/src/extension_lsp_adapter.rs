    ...
    fn get_language_server_command(
        ...
        async move {
            let ret = maybe!(async move {
                ...
                // TODO: This should now be done via the `vela::make_file_executable` function in
                // Vela extension API, but we're leaving these existing usages in place temporarily
                // to avoid any compatibility issues between Vela and the extension versions.
                //
