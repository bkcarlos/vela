...
/// variables to Vela, matching LanguageSettings fields.
...
pub struct ModelineSettings {
    ...
    /// Emacs modeline variables that were parsed but not mapped to Vela settings.
    /// Stored as (variable-name, value) pairs.
    pub emacs_extra_variables: Vec<(String, String)>,
    /// Vim modeline options that were parsed but not mapped to Vela settings.
