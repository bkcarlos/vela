...
//! this binary as a JSON file (path in `VELA_SANDBOX_CHECKS`). Each check
...
mod imp {
    ...
    fn run_checks() -> Result<()> {
        ...
        // effect in a real Vela build.
        // SAFETY: single-threaded at this point.
        unsafe {
            std::env::set_var("VELA_SANDBOX_PROXY_ALLOW_LOCAL_IPS", "1");
        }

        let checks_path =
            std::env::var("VELA_SANDBOX_CHECKS").context("VELA_SANDBOX_CHECKS must be set")?;
        ...
        let echo_port =
            std::env::var("VELA_TEST_ECHO_PORT").unwrap_or_else(|_| DEFAULT_ECHO_PORT.to_string());
