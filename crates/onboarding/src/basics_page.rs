...
use client::{Client, TelemetrySettings, UserStore, vela_urls};
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Vela by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Vela by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Vela can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all Vela's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
    ...
    AgentSetupButton::new(element_id)
        ...
        .on_click(move |_, window, cx| {
            ...
            window.dispatch_action(
                Box::new(vela_actions::agent::SelectAgent {
...
fn render_vela_agent_button(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    let client = Client::global(cx);
    let status = *client.status().borrow();

    let plan = user_store.read(cx).plan();
    let is_free = matches!(plan, Some(Plan::VelaFree) | None);
    let is_pro = matches!(plan, Some(Plan::VelaPro));
    let is_trial = matches!(plan, Some(Plan::VelaProTrial));
    ...
    AgentSetupButton::new("vela-agent-onboarding")
        .icon(
        ...
        .map(|this| {
            if is_signed_in && is_free {
                ...
                this.on_click(move |_, _, cx| {
                    telemetry::event!("Welcome Vela Agent Sign In Clicked");
                    let client = Client::global(cx);
...
fn render_ai_section(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    ...
    let grid = FEATURED_AGENT_IDS.iter().fold(
        div()
            ...
            .gap_2()
            .child(render_vela_agent_button(user_store, cx)),
        |grid, agent_id| {
