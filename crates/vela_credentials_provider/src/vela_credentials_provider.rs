...
///
/// By default, running Vela in development uses the development credentials
...
static VELA_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("VELA_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});

pub struct VelaCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for VelaCredentialsProvider {}
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(VelaCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<VelaCredentialsProvider>()
...
fn new(cx: &App) -> Arc<dyn CredentialsProvider> {
    let use_development_provider = match ReleaseChannel::try_global(cx) {
        Some(ReleaseChannel::Dev) => {
            ...
            //
            // However, if the `VELA_DEVELOPMENT_USE_KEYCHAIN` environment
            // variable is set, we will use the actual keychain.
            !*VELA_DEVELOPMENT_USE_KEYCHAIN
        }
...
/// re-allow access to the system keychain when developing Vela.
