...
/// `VELA_ALLOW_ROOT` environment variable.
#[cfg(unix)]
pub fn prevent_root_execution() {
    let is_root = nix::unistd::geteuid().is_root();
    let allow_root = std::env::var("VELA_ALLOW_ROOT").is_ok_and(|val| val == "true");

    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running Vela as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of Vela.
       It is untested and not recommended, don't complain when things break.
       If you wish to proceed anyways, set `VELA_ALLOW_ROOT=true` in your environment."
        );
        std::process::exit(1);
    }
}
...
/// Returns a shell escaped path for the current vela executable
pub fn get_shell_safe_vela_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
            ...
            .and_then(|n| n.strip_suffix(" (deleted)"))
    {
        ...
        vela_path.set_file_name(truncated);
    }

    vela_path
        .try_shell_safe(shell_kind)
        .context("Failed to shell-escape Vela executable path.")
}

/// Returns a path for the vela cli executable, this function
/// should be called from the vela executable, not vela-cli.
pub fn get_vela_cli_path() -> Result<PathBuf> {
    use anyhow::Context as _;
    let vela_path =
        std::env::current_exe().context("Failed to determine current vela executable path.")?;
    let parent = vela_path
        .parent()
        .context("Failed to determine parent directory of vela executable path.")?;

    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        // On macOS, the vela executable and vela-cli are inside the app bundle,
        // so here ./cli is for both installed and development builds.
        &["./cli"]
    } else if cfg!(target_os = "windows") {
        // bin/vela.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/vela.exe", "./cli.exe"]
        ...
        anyhow::bail!("unsupported platform for determining vela-cli path");
    };
    ...
    possible_locations
        ...
        .find_map(|p| {
            parent
                ...
                .canonicalize()
                .ok()
                .filter(|p| p != &vela_path)
        })
        .with_context(|| {
            format!(
                "could not find vela-cli from any of: {}",
...
pub async fn load_login_shell_environment() -> Result<()> {
    ...
    {
        // Skip SHLVL to prevent it from polluting Vela's process environment.
        // The login shell used for env capture increments SHLVL, and if we propagate it,
        // terminals spawned by Vela will inherit it and increment again, causing SHLVL
