...
/// non-zero exit status even though `vela --printenv` ran successfully and
...
async fn capture_unix(
    ...
    let quoted_vela_path = super::get_shell_safe_vela_path(shell_kind)?;
    ...
    // See: https://github.com/vela-industries/vela/pull/32136#issuecomment-2999645482
    ...
    command_string.push_str(&format!("{} --printenv {}", quoted_vela_path, redir));
...
async fn capture_windows(
    ...
    let vela_path_string = vela_path.display().to_string();
    ...
    let quoted_vela_path = quote_for_shell(&vela_path_string)?;
    let cmd = match shell_kind {
        ...
        | ShellKind::Posix => cmd.args([
            ...
            "-i",
            "-c",
            &format!("cd {}; {} --printenv", quoted_directory, quoted_vela_path),
        ]),
        ShellKind::PowerShell | ShellKind::Pwsh => cmd.args([
            ...
            &format!(
                ...
                quoted_directory, quoted_vela_path
            ),
        ]),
        ...
        ShellKind::Nushell => {
            let vela_command = shell_kind
                .prepend_command_prefix(&quoted_vela_path)
                .into_owned();
            cmd.args([
                "-c",
                &format!("cd {}; {} --printenv", quoted_directory, vela_command),
            ])
        }
        ShellKind::Cmd => {
            let dir = directory_string.trim_end_matches('\\');
            cmd.args(["/d", "/c", "cd", dir, "&&", &vela_path_string, "--printenv"])
        }
    }
    .stdin(Stdio::null())
    .stdout(Stdio::piped())
