...
impl VsCodeDebugTaskDefinition {
    fn try_to_vela(mut self, replacer: &EnvVariableReplacer) -> anyhow::Result<DebugScenario> {
...
impl TryFrom<VsCodeDebugTaskFile> for DebugTaskFile {
    ...
    fn try_from(file: VsCodeDebugTaskFile) -> Result<Self, Self::Error> {
        ...
        let templates = file
            .configurations
            .into_iter()
            .filter_map(|config| config.try_to_vela(&replacer).log_err())
            .collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_parsing_vscode_launch_json() {
        ...
        let vela = DebugTaskFile::try_from(parsed).expect("converting to Vela debug templates");
        pretty_assertions::assert_eq!(
            ...
            DebugTaskFile(vec![DebugScenario {
                ...
                config: json!({
                    "request": "launch",
                    "program": "${VELA_WORKTREE_ROOT}/xyz.js",
                    "showDevDebugOutput": false,
                    "stopOnEntry": true,
                    "args": [
                        "--foo",
                        "${VELA_WORKTREE_ROOT}/thing",
                    ],
                    "cwd": "${VELA_WORKTREE_ROOT}/${FOO}/sub",
                    "env": {
                        "X": "Y",
                    },
                    "type": "node",
