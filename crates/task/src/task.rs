//! Baseline interface of Tasks in Vela: all tasks in Vela are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    Request, TcpArgumentsTemplate, VelaDebugConfig,
};
...
pub use vela_actions::RevealTarget;
...
/// Contains all information needed by Vela to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a Vela's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(VELA_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(VELA_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const VELA_VARIABLE_NAME_PREFIX: &str = "VELA_";
const VELA_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{VELA_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{VELA_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{VELA_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{VELA_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{VELA_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{VELA_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{VELA_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{VELA_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{VELA_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{VELA_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{VELA_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{VELA_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{VELA_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{VELA_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::MainGitWorktree => write!(f, "{VELA_VARIABLE_NAME_PREFIX}MAIN_GIT_WORKTREE"),
            Self::GitSha => write!(f, "{VELA_VARIABLE_NAME_PREFIX}GIT_SHA"),
            Self::GitShaShort => write!(f, "{VELA_VARIABLE_NAME_PREFIX}GIT_SHA_SHORT"),
            Self::GitRepositoryName => write!(f, "{VELA_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_NAME"),
            Self::GitRepositoryPath => write!(f, "{VELA_VARIABLE_NAME_PREFIX}GIT_REPOSITORY_PATH"),
            Self::GitRef => write!(f, "{VELA_VARIABLE_NAME_PREFIX}GIT_REF"),
            Self::Custom(s) => write!(
                f,
                "{VELA_VARIABLE_NAME_PREFIX}{VELA_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Vela at the time the task was spawned.
...
/// Keeps all Vela-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Vela.
...
type VelaEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, VelaEnvVariable>,
    commands: HashMap<VsCodeCommand, VelaEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, VelaEnvVariable>) -> Self {
        Self {
            variables,
    ...
    fn with_commands(
        mut self,
        commands: impl IntoIterator<Item = (VsCodeCommand, VelaEnvVariable)>,
    ) -> Self {
    ...
    // Replaces occurrences of VsCode-specific environment variables with Vela equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Vela hit, perform a substitution
