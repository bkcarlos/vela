//! Headless CLI binary for running Vela's agent in evaluation/benchmark environments.
//!
...
#[command(
    name = "eval-cli",
    about = "Run Vela's agent headlessly in evaluation/benchmark environments"
)]
struct Args {
    /// Output current environment variables as JSON to stdout.
    /// Used internally by Vela's shell environment capture.
    #[arg(long, hide = true)]
    printenv: bool,
...
fn main() {
    ...
    app.run(move |cx| {
        ...
        if let Ok(raw_flags) = std::env::var("VELA_EVAL_ENABLE_FLAGS") {
...
/// the same shape as Vela's `openai_compatible` settings key (provider id ->
/// `{ "api_url": ..., "available_models": [...] }`). Lets vela-eval route the
/// agent itself through an OpenAI-compatible endpoint (e.g. Baseten) that isn't
/// one of Vela's built-in providers, without hardcoding it into eval-cli.
const OPENAI_COMPATIBLE_PROVIDERS_ENV: &str = "VELA_OPENAI_COMPATIBLE_PROVIDERS";
...
/// discovery, in the same shape as Vela's `anthropic.available_models` settings
/// key (a list of `{ "name": ..., "max_tokens": ..., ... }` entries). Lets
/// vela-eval run models that exist on the Anthropic API for the configured
...
const ANTHROPIC_AVAILABLE_MODELS_ENV: &str = "VELA_ANTHROPIC_AVAILABLE_MODELS";
...
async fn run_agent(
    ...
    let setup_result: Result<()> = cx.update(|cx| {
        ...
        let profile_field = {
            let raw = std::env::var("VELA_EVAL_DISABLE_TOOLS").unwrap_or_default();
            let disabled = raw
                .split(',')
