...
def add_build_source_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--clean-source",
        action="store_true",
        help="Build exactly --base-sha/--vela-version with no local patch",
    )
    parser.add_argument(
        "--vela-version",
        help="Git ref/tag/SHA of Vela to build as a clean source snapshot",
    )
...
def add_model_options(parser: argparse.ArgumentParser) -> None:
    ...
    advanced.add_argument(
        "--model-provider",
        choices=("vela", "baseten"),
        default="vela",
        help="Use 'vela' for built-in provider/model ids, or 'baseten' for Baseten Model APIs",
    )
    advanced.add_argument(
        "--baseten-model",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Common workflows:\n"
            "  vela-eval run swe-atlas -m sonnet-4.6       # launch the SWE-Atlas suite\n"
            "  vela-eval run rf -m sonnet-4.6              # launch one benchmark\n"
            "  vela-eval runs                              # what did I launch recently?\n"
            "  vela-eval status                            # check my most recent run\n"
            "  vela-eval status <run-id>                   # check a specific run\n"
            "  vela-eval logs <run-id>                     # print controller logs\n"
            "  vela-eval report <run-id> --fetch           # fetch + score a run\n"
            "\n"
