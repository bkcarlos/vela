"""Harbor installed-agent adapter for Vela's eval-cli binary."""
...
class VelaAgent(BaseInstalledAgent):
    def __init__(
        self,
        logs_dir: Path,
    ...
    async def _install_lsps(self, environment: BaseEnvironment) -> None:
        """Pre-install language servers so Vela doesn't download them at runtime.
        ...
        for name, cmd in lsp_installs:
            try:
                await self.exec_as_agent(
                    environment,
                    command=(
                        'VELA_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/vela"; '
                        + cmd
                    ),
                )
            except Exception as exc:
        ...
        try:
            await self.exec_as_agent(
                ...
                command=(
                    "set -euo pipefail; "
                    'VELA_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/vela"; '
                    'ESLINT_DIR="$VELA_DATA_DIR/languages/eslint/vscode-eslint-2.4.4"; '
                    'mkdir -p "$ESLINT_DIR"; '
                    'curl -fsSL "https://github.com/vela-industries/vscode-eslint/archive/refs/tags/release/2.4.4.tar.gz" '
                    '| tar -xz -C "$ESLINT_DIR"; '
    ...
    def _get_api_env(self) -> dict[str, str]:
        ...
        add_openai_compatible_provider_env(
            env, self._extra_env.get("VELA_OPENAI_COMPATIBLE_PROVIDERS")
        )
        add_anthropic_available_models_env(
            env, self._extra_env.get("VELA_ANTHROPIC_AVAILABLE_MODELS")
        )
        return env
    ...
    def _harbor_agent_budget_sec(self) -> float | None:
        ...
        try:
            ...
            print(
                f"[vela-eval] agent budget: could not read {config_path}: {exc!r}",
                flush=True,
            )
            return None
    ...
    def _task_declared_agent_timeout_sec(self, task_cfg: dict) -> float | None:
        ...
        try:
            ...
            print(
                f"[vela-eval] agent budget: could not parse {task_toml}: {exc!r}",
                flush=True,
            )
            return None
    ...
    def _resolve_task_dir(self, task_cfg: dict) -> Path | None:
        ...
        if name and "/" in name:
            try:
                ...
                print(
                    f"[vela-eval] agent budget: harbor PACKAGE_CACHE_DIR "
                    f"unavailable: {exc!r}",
                    flush=True,
                )
                return None
    ...
    async def run(
        ...
        add_vela_eval_env(
            env, self._extra_env, exclude={"VELA_EVAL_INSTRUCTION_SUFFIX_FILE"}
        )
        ...
        instruction_suffix_file = self._extra_env.get(
            "VELA_EVAL_INSTRUCTION_SUFFIX_FILE"
        )
        ...
        if configured_timeout_raw:
            try:
                ...
                print(
                    "[vela-eval] ignoring non-integer EVAL_CLI_TIMEOUT="
        ...
        if budget_sec is not None:
            ...
            print(
                f"[vela-eval] eval-cli --timeout {timeout_arg}s (harbor agent "
        ...
        await self.exec_as_agent(
            ...
            command=eval_cli_with_log_command(
                ...
                timeout_message=(
                    "[vela-eval] eval-cli timed out (exit 2); continuing to "
