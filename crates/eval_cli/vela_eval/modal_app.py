...
REPO_URL = os.environ.get(
    "AGENT_EVALS_REPO_URL", "https://github.com/vela-industries/vela.git"
)
ZIG_URL = (
...
def build_eval_cli(build_request: dict[str, Any]) -> dict[str, Any]:
    ...
    workdir = pathlib.Path("/build/vela")
    if workdir.exists():
...
def cleanup_artifacts(request: dict[str, Any]) -> dict[str, Any]:
    """On-demand prune (the `vela-eval cleanup` command), supporting --dry-run
...
def run_controller(run_request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        package_file = vela_eval.__file__
        if package_file is None:
            raise RuntimeError("could not resolve vela_eval package location")
