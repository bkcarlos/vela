...
class VelaJudgeProxyVerifier(Verifier):
    ...
    async def _ensure_judge_proxy(self) -> None:
        ...
        if not merged_env.get("VELA_JUDGE_UPSTREAM"):
            return
        ...
        result = await self.environment.exec(
            command=(
                ...
                "cat > /usr/local/lib/vela_judge_proxy.py <<'VELA_PROXY_EOF'\n"
                f"{JUDGE_PROXY_SCRIPT}"
                "VELA_PROXY_EOF\n"
                "cat > /usr/local/bin/ensure-judge-proxy <<'VELA_ENSURE_EOF'\n"
                f"{JUDGE_PROXY_ENSURE_SCRIPT}"
                "VELA_ENSURE_EOF\n"
                "chmod +x /usr/local/bin/ensure-judge-proxy\n"
                "ensure-judge-proxy\n"
                "python3 - <<'VELA_PROXY_CHECK_EOF'\n"
                "import os, socket\n"
                "port = int(os.environ.get('VELA_JUDGE_PROXY_PORT', '8089'))\n"
                "with socket.create_connection(('127.0.0.1', port), timeout=5):\n"
                "    pass\n"
                "VELA_PROXY_CHECK_EOF\n"
                "if [ -f /tmp/vela-judge-proxy.log ]; then "
                "tail -2 /tmp/vela-judge-proxy.log; fi\n"
            ),
            user="root",
            env=env,
        )
