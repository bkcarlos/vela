...
def resolve_source(
    ...
    Precedence: `--from` wins over `--vela-version` / `--base-sha`.
    ...
    vela_version = getattr(args, "vela_version", None)
    if vela_version:
        resolved = source.resolve_remote_ref(repo_url, vela_version)
        return None, True, vela_version, resolved
