...
def build_harness_command(run_request: dict[str, Any], jobs_dir: str) -> list[str]:
    ...
    if not build_id:
        raise ValueError("vela benchmarks require build_id")
    ...
    command = [
        ...
        *dataset_args(benchmark),
        ...
        "--agent-import-path",
        (
            "vela_eval.pier_agent:VelaPierAgent"
            if harness_binary(benchmark) == benchmarks.HARNESS_PIER
            else "vela_eval.agent:VelaAgent"
        ),
        "--ae",
    ...
    if openai_compatible_provider_json:
        command.extend(
            [
                "--ae",
                f"VELA_OPENAI_COMPATIBLE_PROVIDERS={openai_compatible_provider_json}",
            ]
        )
    ...
    if anthropic_available_models_json:
        command.extend(
            [
                "--ae",
                f"VELA_ANTHROPIC_AVAILABLE_MODELS={anthropic_available_models_json}",
            ]
        )
