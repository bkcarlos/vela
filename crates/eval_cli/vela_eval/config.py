...
JUDGE_PROXY_VERIFIER_IMPORT_PATH = "vela_eval.verifier:VelaJudgeProxyVerifier"
...
class JudgeConfig:
    ...
    # judge shim (VELA_JUDGE_MAX_TOKENS). Set for Baseten judges so the runtime
...
def judge_verifier_args(judge: JudgeConfig, judge_model: str) -> list[str]:
    args = [
        ...
        "--ve",
        f"VELA_JUDGE_UPSTREAM={judge.upstream}",
        "--ve",
        f"VELA_JUDGE_AUTH_ENV={judge.auth_env}",
    ]
    if judge.max_tokens is not None:
        args.extend(["--ve", f"VELA_JUDGE_MAX_TOKENS={judge.max_tokens}"])
    return args
