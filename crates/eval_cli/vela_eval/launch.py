...
def command_rejudge(args: argparse.Namespace) -> int:
    ...
    print(f"  vela-eval status {rejudge_request['run_id']}")
    print(f"  vela-eval report {rejudge_request['run_id']} --fetch")
    return 0
...
def launch_prepared_run(
    ...
    print(f"  vela-eval status {run_id}")
    print(f"  vela-eval logs {run_id}")
    print(f"  vela-eval report {run_id} --fetch")
