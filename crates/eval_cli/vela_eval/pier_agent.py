"""Pier-native agent wrapper for Vela's eval-cli binary.
...
class VelaPierAgent(BaseInstalledAgent):
    """Runs Vela's headless eval-cli binary under Pier."""
    ...
    def _container_binary_path(self) -> str:
        ...
        if not path:
            raise ValueError(
                "VelaPierAgent requires EVAL_CLI_CONTAINER_PATH (the eval-cli binary "
    ...
    def network_allowlist(self) -> NetworkAllowlist:
        ...
        providers_json = self._get_env("VELA_OPENAI_COMPATIBLE_PROVIDERS")
        if providers_json:
            try:
    ...
    def _api_env(self) -> dict[str, str]:
        ...
        add_openai_compatible_provider_env(
            env, self._get_env("VELA_OPENAI_COMPATIBLE_PROVIDERS")
        )
        add_anthropic_available_models_env(
            env, self._get_env("VELA_ANTHROPIC_AVAILABLE_MODELS")
        )
        add_vela_eval_env(
    ...
    async def run(
        ...
        instruction_suffix_file = self._get_env("VELA_EVAL_INSTRUCTION_SUFFIX_FILE")
