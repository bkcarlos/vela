...
/// `.vela/settings.json` where `.vela` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.vela`
/// makes `.vela` an intermediate symlink directory.
...
/// either inside a `.vela/` local-settings directory, inside `.agents/skills/`, or inside
...
/// `.vela/` and `.agents/skills/` protections against the canonical form
...
///   2. Intra-project symlinks, e.g. a symlink `safe -> .vela` followed
...
/// like `~/projects/.vela/foo` doesn't classify every file inside it as
/// `.vela/` local-settings — only files that have `.vela` (or
