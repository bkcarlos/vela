...
pub struct SystemPromptTemplate<'a> {
    ...
    pub date: String,
    /// Contents of the user-global `~/.config/vela/AGENTS.md` file (or the
...
mod tests {
    ...
    fn test_system_prompt_template() {
        ...
        assert!(rendered.contains("You are the Vela coding agent"));
    ...
    #[test]
    fn test_system_prompt_does_not_render_legacy_vela_rules_section() {
        let project = prompt_store::ProjectContext::default();
        let template = SystemPromptTemplate {
            project: &project,
            available_tools: vec!["echo".into()],
