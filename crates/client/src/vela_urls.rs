//! Contains helper functions for constructing URLs to various Vela-related pages.
//!
...
/// Returns the URL to the account page on vela.dev.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on vela.dev.
...
/// Returns the URL to the upgrade page on vela.dev.
pub fn upgrade_to_vela_pro_url(cx: &App) -> String {
    format!("{server_url}/account/upgrade", server_url = server_url(cx))
}

/// Returns the URL to Vela's terms of service.
pub fn terms_of_service(cx: &App) -> String {
    format!("{server_url}/terms-of-service", server_url = server_url(cx))
}

/// Returns the URL to Vela AI's privacy and security docs.
...
/// Returns the URL to Vela's edit prediction documentation.
...
/// Returns the URL to Vela's Agent sandboxing documentation.
///
...
/// the page.
///
/// Unlike the account/app links above, this targets `vela.dev/docs` (via
/// [`release_channel::docs_url`]) rather than the configured `server_url`: the
/// docs are a static site hosted on `vela.dev`, so pointing at a local dev
...
/// Returns the URL to Vela's ACP registry blog post.
...
pub fn shared_agent_thread_url(session_id: &str) -> String {
    format!("vela://agent/shared/{}", session_id)
}
