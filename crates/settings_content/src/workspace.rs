...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Vela instance.
    ...
    /// Whether to optimize Vela's interface for assistive technology such as
    /// screen readers.
    ///
    /// Default: false
    pub accessible_mode: Option<bool>,
    /// Whether to use the system provided dialogs for Open and Save As.
    /// When set to false, Vela will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Vela will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Vela will always
    ...
    /// What draws window decorations/titlebar, the client application (Vela) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// Vela draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Vela window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Vela.
    #[default]
    LastSession,
...
pub struct ProjectPanelSettingsContent {
    /// Whether to show the project panel button in the activity bar.
    ///
    /// Default: true
