...
use vela_actions::{
    ...
    OpenServerSettings, OpenSettingsFile, OpenStatusPage, OpenVelaUrl, Quit,
};
use workspace::{
...
const DOCS_URL: &str = "https://vela.dev/docs/";
const STATUS_URL: &str = "https://status.vela.dev";
const MERCH_URL: &str = "https://merch.vela.dev/";
...
pub fn init(cx: &mut App) {
    ...
    // When Vela logs to stdout rather than the log file, avoid registering
    ...
    cx.on_action(|_: &vela_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&vela_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &vela_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    WindowOptions {
        ...
        is_movable: true,
        // Vela draws its own titlebar and moves the window via [`Window::start_window_move`],
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://vela.dev/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vela.dev/docs/linux#could-not-start-inotify");
                    cx.quit();
                });
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://vela.dev/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://vela.dev/docs/windows");
                    cx.quit()
                });
            }
        })
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("VELA_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://vela.dev/docs/windows",
                "https://vela.dev/docs/windows",
            )
        } else {
            (
                "Vulkan",
                "https://vela.dev/docs/linux",
                "https://vela.dev/docs/linux#vela-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            Vela uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set VELA_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenVelaUrl, _, cx| {
        ...
        .register_action(|workspace, action: &vela_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &vela_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &vela_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterVelaScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_vela_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterVelaScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterVelaScheme>(),
                            format!(
                                "vela:// links will now open in {}.",
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &vela_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
                        ...
                        MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                            ...
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    vela_actions::OpenSettingsFile.boxed_clone(),
                                    cx,
                                );
            ...
            if !showed_parse_error {
                show_app_notification(id, cx, move |cx| {
                    cx.new(|cx| {
                        ...
                        .primary_on_click(|window, cx| {
                            window
                                .dispatch_action(vela_actions::OpenSettingsFile.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                    })
                });
...
/// Starts watching `~/.config/vela/AGENTS.md` (or the platform equivalent) and
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(vela_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_worktree_setup_tasks_file(
    workspace: &mut Workspace,
    _: &vela_actions::OpenWorktreeSetupTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    ...
    // Runs automatically after Vela creates a new git worktree.
    // $VELA_WORKTREE_ROOT is the new worktree's root directory, and
    // $VELA_MAIN_GIT_WORKTREE is the original repository's working directory.
    "label": "Set up new worktree",
    "command": "echo \"Setting up $VELA_WORKTREE_ROOT — edit this command\"",
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &vela_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    if let Some(task) = open_local_file(
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .vela/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.vela/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .vela to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".vela".to_string()]);
            });
        });

        eprintln!("Added .vela to file_scan_exclusions in settings");

        // 4. Run tasks to apply settings
        cx.background_executor.run_until_parked();

        // 5. Critical: Verify .vela is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap());

        let has_vela_entry = cx.update(|cx| {
            worktree
                .read(cx)
                .entry_for_path(rel_path(".vela"))
                .is_some()
        });

        eprintln!(
            "Is .vela directory visible in worktree after exclusion: {}",
            has_vela_entry
        );

        // This assertion verifies the test is set up correctly to show the bug
        // If .vela is not excluded, the test will fail here
        assert!(
            !has_vela_entry,
            "Test precondition failed: .vela directory should be excluded but was found in worktree"
        );
    ...
    async fn test_disable_ai_filters_keybindings(cx: &mut gpui::TestAppContext) {
        ...
        let user_binding = KeyBinding::new(
            "ctrl-enter",
            vela_actions::assistant::InlineAssist { prompt: None },
            None,
        );
