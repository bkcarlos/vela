...
use client::{Client, ProxySettings, RefreshLlmTokenListener, UserStore, parse_vela_link};
...
use crate::vela::{CrashHandler, OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn build_application() -> Application {
    ...
    if std::env::var("VELA_EXPERIMENTAL_A11Y").as_deref() == Ok("1") {
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Vela failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Vela failed to open a window: {e:?}. See https://vela.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.vela.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Vela failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://vela.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // args are appended verbatim and would otherwise be misinterpreted as Vela's
    // own arguments.
    ...
    // `vela --crash-handler` Makes vela operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(vela_pid, &output_path, etw_socket.as_str())
        {
    ...
    // `vela --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    #[cfg(target_os = "windows")]
    match util::get_vela_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("VELA_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    let version = option_env!("VELA_BUILD_ID");
    let app_commit_sha =
        option_env!("VELA_COMMIT_SHA").map(|commit_sha| AppCommitSha::new(commit_sha.to_string()));
    ...
    if args.system_specs {
        ...
        println!("Vela System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *vela_env_vars::VELA_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::vela::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::vela::windows_only_instance::handle_single_instance(
                open_listener.clone(),
                &args,
            )
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use vela::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("vela is already running");
        return;
    }
        ...
        Some(
            app.background_executor().spawn(crashes::init(
                InitCrashHandler {
                    ...
                    vela_version: semver::Version::new(
                        app_version.major,
                        app_version.minor,
                        app_version.patch,
                    )
                    .to_string(),
                    binary: "vela".to_string(),
                ...
                |pid| paths::temp_dir().join(format!("vela-crash-handler-{pid}")),
    ...
    app.run(move |cx| {
        ...
        vela::watch_settings_files(fs.clone(), cx);
        ...
        vela::init(cx);
        #[cfg(target_os = "macos")]
        vela::move_to_applications::init(cx);
        ...
        acp_tools::init(cx);
        vela::telemetry_log::init(cx);
        vela::remote_debug::init(cx);
        ...
        vela::watch_user_agents_md(app_state.fs.clone(), cx);
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "vela://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("vela://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // vela://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // vela://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(vela_actions::OpenSettings), cx),
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = vela::open_options_for_request(
    ...
    if let Some(connection_options) = request.remote_connection {
        ...
        let base_open_options = vela::open_options_for_request(open_behavior, &location, cx);
        cx.spawn(async move |cx| {
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Vela would exit silently.
...
#[derive(Parser, Debug)]
#[command(name = "vela", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `vela://` scheme, or relative to <https://vela.dev>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Vela`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/vela`.
    /// On Windows, the default is `%LOCALAPPDATA%\Vela`.
    ...
    /// Instructs vela to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Vela from starting, so you can't run `vela: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Vela run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run vela in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the Vela process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true, allow_hyphen_values = true)]
    etw_vela_pid: Option<i64>,
    ...
    /// Unix socket path for IPC with the parent Vela process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("vela://")
                || arg.starts_with("vela-cli://")
                || arg.starts_with("ssh://")
                || parse_vela_link(arg, cx).is_some()
            {
                arg.into()
            } else {
                format!("file://{arg}")
