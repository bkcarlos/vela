...
use edit_prediction::{EditPredictionModel, VelaEditPredictionDelegate};
...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        EditPredictionProvider::Vela => Some(EditPredictionProviderConfig::Vela(
        ...
        EditPredictionProvider::Ollama | EditPredictionProvider::OpenAiCompatibleApi => {
            ...
            if matches!(format, EditPredictionPromptFormat::Zeta(_)) {
                Some(EditPredictionProviderConfig::Vela(
                    EditPredictionModel::Zeta,
                ))
            } else {
                Some(EditPredictionProviderConfig::Vela(
        ...
        EditPredictionProvider::Mercury => Some(EditPredictionProviderConfig::Vela(
...
impl EditPredictionProviderConfig {
    fn name(&self) -> &'static str {
        match self {
            ...
            EditPredictionProviderConfig::Vela(model) => match model {
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<VelaEditPredictionDelegate>(
        ...
        Some(EditPredictionProviderConfig::Vela(model)) => {
            let ep_store = edit_prediction::EditPredictionStore::global(client, &user_store, cx);
            ...
            if let Some(project) = editor.project() {
                ...
                let provider = cx.new(|cx| {
                    VelaEditPredictionDelegate::new(
                        project.clone(),
                        singleton_buffer,
                        &client,
                        &user_store,
...
mod tests {
    ...
    async fn test_subscribe_uses_stale_provider_config_after_settings_change(
        ...
        // captures None at init time. (The test default is Vela/Zeta1, which
