...
impl MoveToApplicationsRequest {
    ...
    async fn prompt(
        ...
        let response = cx
            .prompt(
                PromptLevel::Info,
                "Move Vela to Applications?",
                Some(
                    "Vela is running from a temporary location. Move it to Applications to finish installing it.",
                ),
                &[
                    PromptButton::ok("Yes"),
            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingVelaModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingVelaModal>(cx) {
                    ...
                    cx.prompt(
                        ...
                        "Failed to move Vela to Applications",
...
pub struct InstallingVelaModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingVelaModal {
...
impl EventEmitter<DismissEvent> for InstallingVelaModal {}

impl ModalView for InstallingVelaModal {
...
impl Focusable for InstallingVelaModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingVelaModal {
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(Label::new("Installing Vela…")),
            )
            .child(
                h_flex()
                    .w_full()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Moving Vela to Applications"))
                            .child(
                                Label::new("Vela will reopen when installation is complete.")
