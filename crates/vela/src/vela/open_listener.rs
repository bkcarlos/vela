...
use client::{VelaLink, parse_vela_link};
...
pub enum OpenRequestKind {
    ...
    InstallSkill {
        /// Full `SKILL.md` contents embedded in a `vela://skill` share link.
        content: String,
    },
    DockMenuAction {
        index: usize,
...
impl OpenRequest {
    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("vela-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("vela-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("vela://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("vela://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("vela://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if url.starts_with(agent_skills::SKILL_SHARE_LINK_PREFIX) {
                this.parse_skill_install_url(&url)?
            } else if let Some(agent_path) = url.strip_prefix("vela://agent") {
                this.parse_agent_url(agent_path)
            } else if url == "vela://" || url == "vela://open" || url == "vela://open/" {
                this.kind = Some(OpenRequestKind::FocusApp);
            } else if let Some(schema_path) = url.strip_prefix("vela://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "vela://settings" || url == "vela://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("vela://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("vela://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("vela://git/commit/") {
                this.parse_git_commit_url(commit_path)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(vela_link) = parse_vela_link(&url, cx) {
                match vela_link {
                    VelaLink::Channel { channel_id } => {
                        this.join_channel = Some(channel_id);
                    }
                    VelaLink::ChannelNotes {
                        channel_id,
                        heading,
                    } => {
    ...
    fn parse_skill_install_url(&mut self, url: &str) -> Result<()> {
        // Format: vela://skill?data=<base64url of SKILL.md contents>
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("vela-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_local_workspace(
    ...
    // `std::env::current_dir()`, since the Vela app process's cwd is typically
...
mod tests {
    use super::*;
    use crate::vela::{open_listener::open_local_workspace, tests::init_test};
    use cli::CliResponse;
    use editor::Editor;
    use futures::poll;
    ...
    fn test_parse_malformed_skill_install_url_errors(cx: &mut TestAppContext) {
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["vela://skill?data=!!!notbase64".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn agent_url_with_prompt(prompt: &str) -> String {
        let mut serializer = url::form_urlencoded::Serializer::new("vela://agent?".to_string());
    ...
    fn test_parse_agent_url_with_trailing_slash(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["vela://agent/?prompt=hello".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_focus_app_url(cx: &mut TestAppContext) {
        ...
        for url in ["vela://", "vela://open", "vela://open/"] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec![url.into()],
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["vela://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["vela://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["vela://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["vela://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Vela to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell Vela to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "vela://git/clone/?repo=https://github.com/vela-industries/vela.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
        ...
        match request.kind {
            Some(OpenRequestKind::GitClone { repo_url }) => {
                assert_eq!(repo_url, "https://github.com/vela-industries/vela.git");
            }
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "vela://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fzed-industries%2Fzed.git"
                            .into(),
                    ],
                    ..Default::default()
                },
    ...
    async fn test_e2e_no_flags_no_windows_no_prompt(cx: &mut TestAppContext) {
        ...
        let (status, prompt_shown) = run_cli_with_vela_handler(
            cx,
            app_state,
