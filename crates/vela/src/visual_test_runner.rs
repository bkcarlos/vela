...
//! Visual Test Runner
//!
//! This binary runs visual regression tests for Vela's UI. It captures screenshots
//! of real Vela windows and compares them against baseline images.
//!
...
//!   cargo run -p vela --bin vela_visual_test_runner --features visual-tests
//!
//! Update baseline images (when UI intentionally changes):
//!   UPDATE_BASELINE=1 cargo run -p vela --bin vela_visual_test_runner --features visual-tests
//!
...
#[cfg(target_os = "macos")]
fn main() {
    // Set VELA_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses vela_env_vars::VELA_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("VELA_STATELESS", "1");
    }
...
mod constants {
    ...
    pub const BASELINE_DIR: &str = "crates/vela/test_fixtures/visual_tests";

    /// Embedded test image (Vela app icon) for visual tests.
...
fn run_visual_tests(project_path: PathBuf, update_baseline: bool) -> Result<()> {
    ...
    // Test settings use Courier font, but we want the real Vela fonts for visual tests
    cx.update(|cx| {
        settings::init(cx);
    });
    ...
    // Initialize all Vela subsystems
    cx.update(|cx| {
        gpui_tokio::init(cx);
...
This is a test project for visual testing of Vela.
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the Vela logo".into()], cx)
    });
...
fn run_tool_permissions_visual_tests(
    ...
    use vela_actions::OpenSettingsAt;
...
fn run_multi_workspace_sidebar_visual_tests(
    ...
    let workspace2_dir = canonical_temp.join("vela");
    ...
    // Add worktree to workspace 2 (index 1) so it shows as "vela"
                ...
                container().child(
                    ThreadItem::new("ti-linked-branch", "Fix scrolling behavior")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/jade-glen/vela".into(),
                ...
                container().child(
                    ThreadItem::new("ti-linked-no-branch", "Review worktree cleanup")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/focal-arrow/vela".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
                            branch_name: None,
                        }]),
            ...
            .child(
                container().child(
                    ThreadItem::new("ti-main-branch", "Request for Long Classic Poem")
                        ...
                        .timestamp("2d")
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            worktree_name: Some("vela".into()),
                            full_path: "/projects/vela".into(),
                            highlight_positions: Vec::new(),
                ...
                container().child(
                    ThreadItem::new("ti-same-name", "Implement feature")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/stoic-reed/vela".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
                ...
                container().child(
                    ThreadItem::new("ti-long-branch", "Overflow test with very long branch")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/my-project/vela".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
                            branch_name: Some(
...
fn run_sidebar_duplicate_project_names_visual_tests(
    ...
    // Create directory structure where every leaf directory is named "vela" but
    ...
    //
    //   code/vela/       — project1 (single worktree)
    //   code/foo/vela/   — project2 (single worktree)
    //   code/bar/vela/   — project3, first worktree
    //   code/baz/vela/   — project3, second worktree
    //
    // No two projects share a worktree path, so ProjectGroupBuilder will
    // place each in its own group.
    let code_vela = canonical_temp.join("code").join("vela");
    let foo_vela = canonical_temp.join("code").join("foo").join("vela");
    let bar_vela = canonical_temp.join("code").join("bar").join("vela");
    let baz_vela = canonical_temp.join("code").join("baz").join("vela");
    std::fs::create_dir_all(&code_vela)?;
    std::fs::create_dir_all(&foo_vela)?;
    std::fs::create_dir_all(&bar_vela)?;
    std::fs::create_dir_all(&baz_vela)?;
    ...
    // Two single-worktree projects whose leaf name is "vela"
    {
        let project1 = create_project_with_worktree(&code_vela, &app_state, cx)?;
        let project2 = create_project_with_worktree(&foo_vela, &app_state, cx)?;
    ...
    // Three projects, third has two worktrees (all leaf names "vela")
    //
    // project1: code/vela
    // project2: code/foo/vela
    // project3: code/bar/vela + code/baz/vela
    //
    ...
    // separate groups. The sidebar must disambiguate all three.
    {
        ...
        let project3 = create_project_with_worktree(&bar_vela, &app_state, cx)?;
        let add_second_worktree = cx.update(|cx| {
            project3.update(cx, |project, cx| {
                project.find_or_create_worktree(&baz_vela, true, cx)
            })
        });
        cx.background_executor.allow_parking();
        cx.foreground_executor
