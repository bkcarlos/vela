//! The Vela Rust Extension API allows you write extensions for [Vela](https://vela.dev/) in Rust.
...
pub use wit::{
    ...
    vela::extension::github::{
        GithubRelease, GithubReleaseAsset, GithubReleaseOptions, github_release_by_tag_name,
        latest_github_release,
    },
    vela::extension::nodejs::{
        node_binary_path, npm_install_package, npm_package_installed_version,
        npm_package_latest_version,
    },
    vela::extension::platform::{Architecture, Os, current_platform},
    vela::extension::slash_command::{
...
pub mod lsp {
    pub use crate::wit::vela::extension::lsp::{
        Completion, CompletionKind, InsertTextFormat, Symbol, SymbolKind,
    };
}

/// A result returned from a Vela extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a Vela task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, Vela will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent Vela from running second phase of resolution in such case.
...
/// Registers the provided type as a Vela extension.
///
...
macro_rules! register_extension {
    ($extension_type:ty) => {
        ...
        pub extern "C" fn __init_extension() {
            ...
            vela_extension_api::register_extension(|| {
                Box::new(<$extension_type as vela_extension_api::Extension>::new())
            });
        }
    };
}
...
#[unsafe(link_section = "vela:api-version")]
#[doc(hidden)]
pub static VELA_API_VERSION: [u8; 6] = *include_bytes!(concat!(env!("OUT_DIR"), "/version_bytes"));
