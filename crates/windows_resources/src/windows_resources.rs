...
fn git_sha() -> Option<String> {
    if let Ok(sha) = std::env::var("VELA_COMMIT_SHA") {
        return Some(sha);
    }
...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../vela/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Vela"),
        "preview" => ("app-icon-preview.ico", "Vela Preview"),
        "nightly" => ("app-icon-nightly.ico", "Vela Nightly"),
        _ => ("app-icon-dev.ico", "Vela Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "Vela Industries, Inc.\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 Vela Industries, Inc.\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    let rc_path = out_dir.join("vela_resources.rc");
    std::fs::write(&rc_path, rc_content)?;

    if let Ok(toolkit_path) = std::env::var("VELA_RC_TOOLKIT_PATH") {
