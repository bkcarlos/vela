...
pub struct RootPlan {
    ...
    /// recorded when Vela created the worktree. [`remove_root`] re-stats the
    /// directory and refuses to delete anything if the time has changed,
    /// which means the worktree was recreated outside Vela.
...
/// Resolves the Vela-managed worktrees base directory for a given repo.
///
/// This intentionally reads the *global* `git.worktree_directory` setting
/// rather than any project-local override, because Vela always uses the
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the Vela-managed worktrees
    ...
    // Only archive worktrees that Vela explicitly created. The directory
    // check above constrains paths, but the database record is what
    // distinguishes a Vela-created worktree from one the user manually
...
pub async fn remove_root(root: RootPlan, cx: &mut AsyncApp) -> Result<()> {
    verify_created_by_vela(&root, cx).await?;
    ...
    // user later creates a new worktree at the same path outside Vela, a
...
/// Confirms that the worktree on disk is still the one Vela created, by
/// comparing the creation time of its git metadata directory against the
/// time recorded when Vela created it.
///
/// Outcomes:
...
///   Vela. The registry record is removed (so subsequent archival attempts
...
async fn verify_created_by_vela(root: &RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    let created_at = receiver
        ...
        .with_context(|| {
            format!(
                "refusing to delete worktree at {}: failed to verify that Vela created it",
    ...
    match created_at {
        ...
        Some(_) => {
            ...
            .log_err();
            Err(anyhow!(
                "refusing to delete worktree at {}: it is not the worktree Vela created \
                 (it was likely removed and recreated outside Vela)",
                root.root_path.display()
            ))
        }
    }
...
pub async fn restore_worktree_via_git(
    ...
    if created_new_worktree {
        // Re-register the restored worktree as Vela-created so it can be
...
mod tests {
    ...
    async fn record_vela_created_worktree(
        fs: &FakeFs,
        worktree_path: &Path,
        cx: &mut TestAppContext,
    ) {
        crate::test_support::record_vela_created_worktree(fs, worktree_path, None, cx).await
    }
    ...
    async fn test_build_root_plan_returns_some_for_linked_worktree(cx: &mut TestAppContext) {
        ...
        .await;
        record_vela_created_worktree(&fs, Path::new("/worktrees/project/feature/project"), cx)
            .await;
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the Vela-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_build_root_plan_returns_none_for_unrecorded_linked_worktree_in_managed_directory(
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                plan.is_none(),
                "build_root_plan should return None for a linked worktree Vela didn't create, \
                 even when it lives inside the Vela-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_build_root_plan_with_custom_worktree_directory(cx: &mut TestAppContext) {
        ...
        // It is recorded as Vela-created so that the directory check, not
        ...
        .await;
        record_vela_created_worktree(&fs, Path::new("/worktrees/project/feature2/project"), cx)
            .await;
    ...
    #[gpui::test]
    async fn test_remove_root_refuses_when_worktree_recreated_outside_vela(
        cx: &mut TestAppContext,
    ) {
        init_test(cx);
        ...
        // simulating a worktree that was removed and recreated outside Vela
        // after Vela recorded the original.
        ...
        assert!(
            error.to_string().contains("not the worktree Vela created"),
    ...
    async fn test_remove_root_returns_error_and_rolls_back_on_remove_dir_failure(
        ...
        // worktree itself intact so the created-by-Vela verification passes.
        let worktree_path = Path::new("/worktrees/project/feature/project");
