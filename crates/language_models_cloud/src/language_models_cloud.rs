...
use cloud_llm_client::{
    ...
    SERVER_SUPPORTS_STATUS_MESSAGES_HEADER_NAME, VELA_VERSION_HEADER_NAME,
};
use futures::{
...
use language_model::{
    ...
    OPEN_AI_PROVIDER_NAME, RateLimiter, VELA_CLOUD_PROVIDER_ID, VELA_CLOUD_PROVIDER_NAME,
...
/// Sends an authenticated request to the Vela LLM service, retrying once with
...
impl<TP: CloudLlmTokenProvider> CloudLanguageModel<TP> {
    pub async fn perform_llm_completion(
        ...
        let url = http_client
            .build_vela_llm_url("/completions", &[])
            ...
            authenticated_llm_request(http_client, token_provider, auth_context, |token| {
                Ok(http_client::Request::builder()
                    ...
                    .when_some(app_version.as_ref(), |builder, app_version| {
                        builder.header(VELA_VERSION_HEADER_NAME, app_version.to_string())
                    })
...
/// Represents error responses from Vela's cloud API.
///
...
impl<TP: CloudLlmTokenProvider + 'static> CloudModelProvider<TP> {
    ...
    async fn fetch_models_request(
        ...
        let url = http_client.build_vela_llm_url("/models", &[])?;
        let mut response =
            authenticated_llm_request(http_client, token_provider, auth_context, |token| {
                Ok(http_client::Request::builder()
                    .method(Method::GET)
...
mod tests {
    ...
    fn test_api_error_conversion_with_upstream_http_error() {
        ...
        // Regular 500 error without upstream_http_error should remain ApiInternalServerError for Vela
