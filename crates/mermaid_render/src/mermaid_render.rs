...
//! pipeline. Vela opts into that pipeline during rendering, then keeps
...
//! - [`postprocess`] the SVG to add Vela theme and accent styling.
//!
//! Vela's postprocessing is split up into stages. We parse the generated SVG
...
//! `player_colors` in the Vela theme.
//!
...
//!    `vela-accent-0`) to specific elements, based on the diagram type and
//!    node.
...
/// from Vela's UI palette) round-trip without silently losing their alpha.
