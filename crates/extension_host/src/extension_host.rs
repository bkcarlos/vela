...
/// The current extension [`SchemaVersion`] supported by Vela.
...
/// Returns the [`SchemaVersion`] range that is compatible with this version of Vela.
pub fn schema_version_range() -> RangeInclusive<SchemaVersion> {
    SchemaVersion::ZERO..=CURRENT_SCHEMA_VERSION
}

/// Returns whether the given extension version is compatible with this version of Vela.
...
impl ExtensionStore {
    ...
    /// Installs any extensions that should be included with Vela by default.
    ///
    ...
    fn fetch_extensions_from_api(
        ...
        let url = self.http_client.build_vela_api_url(path, query);
