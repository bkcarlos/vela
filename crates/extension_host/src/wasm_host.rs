...
impl extension::Extension for WasmExtension {
    ...
    async fn dap_config_to_scenario(&self, config: VelaDebugConfig) -> Result<DebugScenario> {
        self.call(|extension, store| {
            async move {
                let kind = extension
    ...
    pub fn load_extension(
        ...
        let compile_task = {
            ...
            executor.spawn(async move {
                let vela_api_version = parse_wasm_extension_version(&manifest_id, &wasm_bytes)?;
                let component = Component::from_binary(&engine, &wasm_bytes)
                    .context("failed to compile wasm component")?;

                anyhow::Ok((vela_api_version, component))
            })
        };

        let load_extension = |vela_api_version: Version, component| async move {
        ...
        cx.spawn(async move |cx| {
            let (vela_api_version, component) = compile_task.await?;

            // Run wasi-dependent operations on tokio.
            // wasmtime_wasi internally uses tokio for I/O operations.
            let (extension_task, manifest, work_dir, tx, vela_api_version) =
                gpui_tokio::Tokio::spawn(cx, load_extension(vela_api_version, component)).await??;
...
pub fn parse_wasm_extension_version(extension_id: &str, wasm_bytes: &[u8]) -> Result<Version> {
    ...
    for part in wasmparser::Parser::new(0).parse_all(wasm_bytes) {
        if let wasmparser::Payload::CustomSection(s) =
            ...
            && s.name() == "vela:api-version"
        {
            version = parse_wasm_extension_version_custom_section(s.data());
            if version.is_none() {
                bail!(
                    "extension {} has invalid vela:api-version section: {:?}",
                    extension_id,
                    s.data()
                );
            }
    ...
    version.with_context(|| format!("extension {extension_id} has no vela:api-version section"))
}
