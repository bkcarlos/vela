//! Paths to locations used by Vela.
...
///
/// Forks should change this to avoid colliding with Vela's user data.
...
/// Returns the relative path to the vela_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".vela_server").unwrap());
    *CACHED
}

// Remove this once 223 goes stable
/// Returns the relative path to the vela_wsl_server directory on the wsl host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".vela_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by Vela.
...
/// Returns the path to the data directory used by Vela.
...
/// Returns the path to the temp directory used by Vela.
...
/// Returns the path to the Vela server directory on this SSH host.
...
/// Returns the path to the `Vela.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the `Vela.log.old` file.
...
/// user opens, and is loaded into the native Vela agent's system prompt.
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Vela repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Vela.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Vela.
...
/// Returns the relative path to a `.vela` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".vela/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".vela/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::from_unix_str(".vela/debug.json").unwrap());
    *CACHED
}
