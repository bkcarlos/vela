...
use vela_actions::{OpenBrowser, OpenSettingsAt};
use workspace::{
...
const PRIVACY_DOCS: &str = "https://vela.dev/docs/ai/privacy-and-security";
...
impl Render for EditPredictionButton {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match language_settings.edit_predictions.provider {
            ...
            provider @ (EditPredictionProvider::Vela | EditPredictionProvider::Mercury) => {
                let enabled = self.editor_enabled.unwrap_or(true);
                let file = self.file.clone();
                let language = self.language.clone();
                let project = self.project.clone();
                let provider_name: &'static str = match provider {
                    EditPredictionProvider::Vela => "vela",
                    _ => "unknown",
                };
                let icons = self
                ...
                let vela_cloud_needs_sign_in =
                    matches!(provider, EditPredictionProvider::Vela) && user.is_none();
                let provider_unavailable =
                    missing_token || mercury_has_error || vela_cloud_needs_sign_in;

                let icon_button = IconButton::new("vela-predict-pending-button", ep_icon)
                    .shape(IconButtonShape::Square)
                    .tab_index(0isize)
                    .aria_label("Edit Prediction")
...
impl EditPredictionButton {
    ...
    fn add_provider_switching_section(
        ...
        let is_vela_provider_disabled = organization_configuration
        ...
        if !providers.is_empty() {
            ...
            for provider in providers {
                ...
                let is_disabled_vela_provider =
                    provider == EditPredictionProvider::Vela && is_vela_provider_disabled;
                ...
                menu = menu.item(
                    ContextMenuEntry::new(name)
                        .toggleable(
                            IconPosition::Start,
                            is_current && !is_disabled_vela_provider,
                        )
                        .disabled(is_disabled_vela_provider)
                        .when(is_disabled_vela_provider, |item| {
    ...
    pub fn build_language_settings_menu(
        ...
        if matches!(provider, EditPredictionProvider::Vela) {
                                ...
                                v_flex()
                                    ...
                                    .child(
                                        Label::new(indoc!{
                                            ...
                                            Vela must detect a license file in your repo for this setting to take effect. \
        ...
        menu = menu.item(
            ContextMenuEntry::new("Configure Excluded Files")
                ...
                .documentation_aside(DocumentationSide::Left, |_| {
                    Label::new(indoc!{"
                        Open your settings to add sensitive paths for which Vela will never predict edits."}).into_any_element()
                })
                .handler(move |window, cx| {
                    telemetry::event!(
                        "Edit Prediction Menu Action",
    ...
    fn build_edit_prediction_context_menu(
        ...
        ContextMenu::build(window, cx, |mut menu, window, cx| {
            ...
            let needs_sign_in = user.is_none()
                && matches!(
                    provider,
                    EditPredictionProvider::None | EditPredictionProvider::Vela
                );
            ...
            if needs_sign_in {
                    ...
                    .and_then(|provider| provider.usage(cx))
                {
                    ...
                    menu = menu
                        ...
                        .entry(
                            "Check your payment status or contact us at billing-support@vela.dev to continue using this feature.",
                            None,
                            |_window, cx| {
                                cx.open_url(&vela_urls::account_url(cx))
                            },
...
pub fn get_available_providers(cx: &mut App) -> Vec<EditPredictionProvider> {
    ...
    providers.push(EditPredictionProvider::Vela);
